...
def locate_bundle():
    ...
    candidates.extend(parent / "ppp-round-2" for parent in (here, *here.parents))
    candidates.extend([
    ...
    raise FileNotFoundError("The official ppp-round-2 train.csv/test.csv bundle was not found")

DATA_DIR = locate_bundle()
OUTPUT_PATH = Path.cwd() / "submission.csv"
RUNTIME_DIR = Path.cwd() / ".final_submission_runtime"
RUNTIME_DIR.mkdir(parents=True, exist_ok=True)
print("data bundle:", DATA_DIR)
print("python:", platform.python_version())
print("output:", OUTPUT_PATH)
print("official input hashes:")
...
def canonicalize(value):
    ...
    if molecule is None:
        raise ValueError(f"RDKit could not parse official SMILES: {value}")
...
all_keys = sorted(set(train.canonical) | set(test.canonical) | set(archive.canonical))
...
archive["structure_index"] = archive.canonical.map(key_index).astype(int)

n_structures = len(all_keys)
labels = np.full((n_structures, len(TARGETS)), np.nan, dtype=np.float64)
archive_only_labels = np.full((n_structures, len(TARGETS)), np.nan, dtype=np.float64)
archive_grouped = archive.groupby(["structure_index", "target_type"], as_index=False)["target"].mean()
for row in archive_grouped.itertuples(index=False):
    archive_only_labels[int(row.structure_index), TARGET_INDEX[row.target_type]] = float(row.target)
...
summary = pd.DataFrame({
    "target": TARGETS,
    "current_rows": [int((train.target_type == target).sum()) for target in TARGETS],
    "archive_rows": [int((archive.target_type == target).sum()) for target in TARGETS],
    "canonical_labelled_structures": [int(observed[:, j].sum()) for j in range(len(TARGETS))],
    "test_rows": [int((test.target_type == target).sum()) for target in TARGETS],
})
display(summary)
...
print("PI1M rows available:", (sum(1 for _ in pi1m_path.open()) - 1) if pi1m_path.is_file() else "not mounted")
...
def topology_block(mols, smiles):
    names = [
        "smiles_length", "atom_count", "heavy_atom_count", "dummy_count", "ring_count",
        "aromatic_atoms", "hetero_atoms", "halogen_atoms", "rotatable_bonds", "double_bonds",
        "triple_bonds", "branch_count", "formal_charge", "molecular_weight", "mol_logp",
        "molar_refractivity", "tpsa", "labute_asa", "fraction_csp3", "hba", "hbd",
        "gasteiger_min", "gasteiger_max", "gasteiger_mean_abs", "gasteiger_std",
        "conjugated_atoms", "longest_conjugated_path", "backbone_path_atoms", "side_atoms",
        "star_count", "star_neighbor_atomic_sum", "weighted_bond_sum", "aromatic_bond_count",
        "hetero_fraction", "heavy_atom_density", "ring_density", "oxygen_count", "nitrogen_count",
        "silicon_count", "sulfur_count", "phosphorus_count", "fluorine_count", "chlorine_count",
        "bromine_count", "iodine_count", "spectral_gap", "spectral_top", "spectral_bottom",
    ]
    rows = []
...
TOPO, TOPO_NAMES = topology_block(molecules, all_keys)
...
# Exact canonical overrides are derived only from official labelled rows.
unique_values = {}
combined_labels = pd.concat([archive, train], ignore_index=True)
for (canonical, target), group in combined_labels.groupby(["canonical", "target_type"]):
...
print("official canonical overrides:", dict(override_counts))
...
for target in TARGETS:
    ...
    local = score_r2(y, p)
    if target in ("tg", "egc"):
        coverage = np.mean([np.isfinite(labels[int(fi), j]) for fi in test.loc[test.target_type == target, "structure_index"]])
        archive_free = ~np.isfinite(archive_only_labels[rows, j])
        if archive_free.sum() > 20:
            residual_mse = np.mean((y[archive_free] - p[archive_free]) ** 2)
        else:
            residual_mse = np.mean((y - p) ** 2)
        variance_proxy = np.var(y)
        estimate = 1.0 - (1.0 - coverage) * residual_mse / max(variance_proxy, 1e-12)
    else:
        coverage = np.nan
        estimate = local
    expected[target] = float(estimate)
    final_rows.append({"target": target, "selected_oof_r2": local, "test_exact_coverage": coverage, "expected_test_r2": estimate})
expected_table = pd.DataFrame(final_rows)
expected_table.loc[len(expected_table)] = {"target": "mean", "selected_oof_r2": expected_table.selected_oof_r2.mean(), "test_exact_coverage": np.nan, "expected_test_r2": expected_table.expected_test_r2.mean()}
display(expected_table)
print("expected arithmetic mean:", float(np.mean(list(expected.values()))))
print("submission path:", OUTPUT_PATH.resolve())
print("submission sha256:", sha256_file(OUTPUT_PATH))
print("submission schema:", list(submission.columns), submission.shape)
