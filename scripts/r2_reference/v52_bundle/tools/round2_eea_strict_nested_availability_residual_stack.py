#!/usr/bin/env python3
"""Strictly nested Eea availability-gated residual diagnostic.

This is a new child of the stopped C034 attempt. It reuses only the generic
feature/model plumbing while repairing the validation boundary: inner
auxiliary fits exclude outer and inner groups, and all scaffold holdout fits
exclude the held-out scaffolds.
"""
...
ORIGINAL_AUX_PREDICTION = implementation.aux_prediction
...
def strict_aux_prediction(target, forbidden_groups, prediction_global, aux_info, deterministic):
    ...
    global_forbidden_groups = set(forbidden_groups) | CURRENT_OUTER_GROUPS
    candidate_global = info["global_indices"]
    keep = np.asarray([
        group not in global_forbidden_groups
...
def strict_nested_split(
    ...
    parent = np.asarray(result["parent"], dtype=np.float64)
    candidate = np.asarray(result["candidate"], dtype=np.float64).copy()
    candidate[~eligible] = parent[~eligible]
    ...
    if CALL_INDEX < 5:
        MAIN_ROUTE_ROWS.append({
            ...
            "available_count": np.asarray(available_count, dtype=np.int64),
...
def main() -> None:
    ...
    metrics["schema_version"] = "ppp.round2.eea-strict-nested-availability-residual-stack-run.v1"
    metrics["parent"] = "R2-C034-20260803-2000-eea-availability-gated-residual-stack"
    metrics["target"] = "eea"
    metrics["auxiliary_targets"] = list(implementation.AUXILIARY)
    rows = {
        key: np.concatenate([item[key] for item in MAIN_ROUTE_ROWS])
        for key in ("y", "parent", "candidate", "available_count", "nearest")
    }
    low_similarity = rows["nearest"] < 0.30
    low_auxiliary = rows["available_count"] < 2
    eligible = ~low_similarity & ~low_auxiliary
    route_definition = {
        ...
        "low_auxiliary_rows": int(np.sum(low_auxiliary)),
        "low_similarity_changed_rows": int(np.sum(low_similarity & (np.abs(rows["candidate"] - rows["parent"]) > 1.0e-12))),
        "low_auxiliary_changed_rows": int(np.sum(low_auxiliary & (np.abs(rows["candidate"] - rows["parent"]) > 1.0e-12))),
    }
    route_definition["auxiliary_eligible_delta"] = route_delta(rows, eligible)
    route_definition["low_similarity_control_delta"] = 0.0 if route_definition["low_similarity_changed_rows"] == 0 else None
    route_definition["low_auxiliary_control_delta"] = 0.0 if route_definition["low_auxiliary_changed_rows"] == 0 else None
    metrics["route_definition"] = route_definition
    metrics["panels"]["availability_declared_auxiliaries"] = {
        "rows": int(np.sum(eligible | low_auxiliary)),
        "delta_r2": route_definition["auxiliary_eligible_delta"],
        "status": "evaluable" if route_definition["auxiliary_eligible_delta"] is not None else "insufficient_or_constant",
    }
    metrics["panels"]["availability_fewer_than_two_declared_auxiliaries"] = {
        "rows": int(np.sum(low_auxiliary)),
        "delta_r2": route_definition["low_auxiliary_control_delta"],
        "status": "unchanged_control",
    }
    ...
    metrics["pass"] = bool(
        ...
        and route_definition["auxiliary_eligible_delta"] is not None
        and route_definition["auxiliary_eligible_delta"] >= -0.003
        and route_definition["low_similarity_changed_rows"] == 0
        and route_definition["low_auxiliary_changed_rows"] == 0
    )
    ...
    config["schema_version"] = "ppp.round2.eea-strict-nested-availability-residual-stack.v1"
    config["target"] = "eea"
    config["auxiliary_targets"] = list(implementation.AUXILIARY)
    config["strict_nested_boundary"] = True
    ...
    (run_dir / "run.log").write_text(
        ...
        f"eligible_rows={route_definition['eligible_rows']}\n"
        f"low_similarity_changed_rows={route_definition['low_similarity_changed_rows']}\n"
        f"low_auxiliary_changed_rows={route_definition['low_auxiliary_changed_rows']}\n"
        f"pass={metrics['pass']}\n",
        encoding="utf-8",
    )
    manifest = []
    ...
    manifest.extend([
        f"{metrics['source_hashes']['wrapper']}  SOURCE tools/round2_eea_strict_nested_availability_residual_stack.py",
