...
def fold_matrix(matrix: np.ndarray, train_index: np.ndarray, validation_index: np.ndarray) -> tuple[np.ndarray, np.ndarray, int]:
    ...
    invalid_train = ~np.isfinite(train_x) | (np.abs(train_x) > float(reference.DEFAULT_CONFIG["dense_abs_limit"]))
    invalid_validation = ~np.isfinite(validation_x) | (np.abs(validation_x) > float(reference.DEFAULT_CONFIG["dense_abs_limit"]))
    train_x[invalid_train] = np.nan
    validation_x[invalid_validation] = np.nan
