#!/usr/bin/env python3
"""Bounded official-only Eea low-rank cross-property calibration diagnostic."""
...
TARGET = "eea"
CROSS_TARGETS = ("egb", "egc", "nc", "eps", "ei")
...
def fit_candidate(
    ...
    cross_train = np.asarray(cross_matrix[global_indices[train]], dtype=np.float64).copy()
    cross_validation = np.asarray(cross_matrix[global_indices[validation]], dtype=np.float64).copy()
    cross_imputer = SimpleImputer(strategy="median", keep_empty_features=True)
    cross_scaler = StandardScaler()
    cross_train = cross_scaler.fit_transform(cross_imputer.fit_transform(cross_train))
    cross_validation = cross_scaler.transform(cross_imputer.transform(cross_validation))
    pca = PCA(n_components=2, svd_solver="full", random_state=2026)
    cross_train = pca.fit_transform(cross_train)
    cross_validation = pca.transform(cross_validation)
    det_train = np.asarray(deterministic[global_indices[train]], dtype=np.float64).copy()
    det_validation = np.asarray(deterministic[global_indices[validation]], dtype=np.float64).copy()
    det_imputer = SimpleImputer(strategy="median", keep_empty_features=True)
    det_scaler = StandardScaler()
    det_train = det_scaler.fit_transform(det_imputer.fit_transform(det_train))
    det_validation = det_scaler.transform(det_imputer.transform(det_validation))
    train_x = np.hstack([cross_train, det_train])
    validation_x = np.hstack([cross_validation, det_validation])
    model = BayesianRidge(alpha_1=1.0e-6, alpha_2=1.0e-6, lambda_1=1.0e-6, lambda_2=1.0e-6, fit_intercept=True)
    model.fit(train_x, y[train])
    return reference.clip_prediction(y[train], model.predict(validation_x))
...
def main() -> None:
    ...
    descriptor_map = {name: descriptor[:, index] for index, name in enumerate(descriptor_names)}
    physical_map = {name: physical[:, index] for index, name in enumerate(physical_names)}
    deterministic_names = ["MolMR", "MolLogP", "TPSA", "FractionCSP3", "NumHAcceptors"]
    deterministic_parts = [descriptor_map[name] for name in deterministic_names if name in descriptor_map]
    deterministic_names = [name for name in deterministic_names if name in descriptor_map]
    for name in ("aromatic_atom_count", "hetero_atom_count", "heavy_atom_count", "ring_count"):
        deterministic_parts.append(physical_map[name])
        deterministic_names.append(name)
    deterministic = np.column_stack(deterministic_parts).astype(np.float64, copy=False)
    cross_values, cross_available = reference.cross_property_arrays(pooled, keys)
    cross_indices = [reference.TARGETS.index(name) for name in CROSS_TARGETS]
    cross_matrix = np.hstack([cross_values[:, cross_indices], cross_available[:, cross_indices]]).astype(np.float64, copy=False)
    ...
    for fold in range(5):
        ...
        prediction = fit_candidate(cross_matrix, deterministic, y, training, validation, indices)
    ...
    script_path = root / "tools" / "round2_eea_low_rank_cross_property_calibration.py"
    reference_path = root / "tools" / "initial_reference_pipeline.py"
    audit = {
        "schema_version": "ppp.round2.eea-low-rank-cross-property-calibration-run.v1",
        "experiment_id": run_dir.name,
        "created_at": datetime.now().astimezone().isoformat(),
        "parent": "R2-C029-20260803-2200-eps-scaffold-balanced-pls",
        ...
        "candidate_r2_low_rank_bayesian_ridge": candidate_r2,
        "delta_r2": delta_r2,
    ...
    write_json(run_dir / "config.json", {"schema_version": "ppp.round2.eea-low-rank-cross-property-calibration.v1", "seed": 2026, "target": TARGET, "outer_folds": 5, "inner_folds": 4, "cross_targets": CROSS_TARGETS, "pca_components": 2, "estimator": {"name": "BayesianRidge", "alpha_1": 1.0e-6, "alpha_2": 1.0e-6, "lambda_1": 1.0e-6, "lambda_2": 1.0e-6}, "deterministic_names": deterministic_names, "official_inputs": inputs, "source_hashes": audit["source_hashes"]})
    ...
    manifest.extend([f"{audit['source_hashes']['script']}  SOURCE tools/round2_eea_low_rank_cross_property_calibration.py", f"{audit['source_hashes']['reference_module']}  SOURCE tools/initial_reference_pipeline.py"])
