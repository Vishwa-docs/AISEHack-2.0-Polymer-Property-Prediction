#!/usr/bin/env python3
"""C195: fixed Nc residual-diversity test from two regenerated near-miss carriers.

This is a bounded official-only Nc child queued behind C194 and before C193.
It does not replay stored predictions.  It rebuilds the exact C050 parent,
regenerates the C180 clean near-miss Nc carrier from source, regenerates a
CatBoost-free physical/electronic Nc carrier from the C129 feature family, and
tests one fixed equal-weight ensemble:

  Nc_candidate = 0.5 * C180_FloryFox_Nc + 0.5 * PhysElec_HGB_ET_Nc

The scientific question is whether two independent Nc-positive but individually
unbanked residual carriers have enough error diversity to clear the standard
component gate.  If the fixed ensemble misses any gate, no target is banked and
C193 must keep the exact C050 Nc fallback.
"""
...
def nc_test_indices(parent: dict[str, Any]) -> tuple[pd.DataFrame, np.ndarray, np.ndarray]:
    ...
    if not np.array_equal(test_rows["id"].to_numpy(int), test_detail["id"].to_numpy(int)):
        raise RuntimeError("C195 Nc test ID alignment failed")
...
def main() -> None:
    ...
    if len(fixed_candidate) != len(parent_oof) or len(fixed_test) != len(test_rows):
        raise RuntimeError("C195 fixed ensemble length mismatch")

    active_report = carrier.evaluate_target(info, {"candidate": fixed_candidate})
    active_report.update(
        {
            "ensemble_rule": "0.5*C180_FloryFox_Nc + 0.5*PhysElec_HGB_ET_Nc",
    ...
    if (
        ...
        raise RuntimeError("C195 complete output contract failed")

    report = {
        "schema_version": "ppp.round2.c195.nc-nearmiss-residual-diversity.v1",
        "experiment_id": run_dir.name,
        "created_at": pd.Timestamp.now(tz="Asia/Kolkata").isoformat(),
        "parent": "C050 source rebuild; no stored C180/C129 prediction input",
        "hypothesis": "A fixed equal-weight ensemble of regenerated C180 Flory-Fox Nc and a CatBoost-free physical/electronic HGB-ExtraTrees Nc carrier can bank Nc by residual diversity.",
    ...
    write_json(
        run_dir / "config.json",
        {
            ...
            "ensemble_rule": "fixed_equal_weight",
