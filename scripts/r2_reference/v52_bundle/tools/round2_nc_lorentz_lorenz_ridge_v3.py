...
def masked_target_dense(base_dense, cross_values, cross_available, target):
    # Preserve the exact v7/C001 parent feature construction. The specialist
    # itself uses only fixed physical descriptors, but the parent comparison
    # must retain its official cross-property covariates for parity.
...
def main() -> None:
    ...
    source_hashes = {"script": sha256_file(Path(__file__).resolve()), "reference_module": sha256_file(root / "tools" / "initial_reference_pipeline.py"), "bootstrap_plumbing": sha256_file(root / "tools" / "round2_eea_cross_target_oof_residual_stack.py")}
    ...
    manifest.extend([f"{source_hashes['script']}  SOURCE tools/round2_nc_lorentz_lorenz_ridge_v3.py", f"{source_hashes['reference_module']}  SOURCE tools/initial_reference_pipeline.py", f"{source_hashes['bootstrap_plumbing']}  SOURCE tools/round2_eea_cross_target_oof_residual_stack.py"])
