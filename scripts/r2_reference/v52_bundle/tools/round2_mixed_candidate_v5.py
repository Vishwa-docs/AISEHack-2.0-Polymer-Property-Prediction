...
def unchanged_panel_report(
    ...
    # Match the C001 target-local KFold support: each row is compared only to
    # structures in its own target's training partition, never to test or other
    # target structures.
    similarity = np.full(len(rows), np.nan, dtype=np.float64)
    kfold = KFold(n_splits=5, shuffle=True, random_state=2026)
    all_rows = np.arange(len(rows), dtype=np.int64)
    for _, validation in kfold.split(all_rows):
        training = np.setdiff1d(all_rows, validation, assume_unique=True)
        similarity[validation] = ei_route.nearest_similarity(
            fingerprints, row_indices[validation], row_indices[training]
        )
    ...
    for name, selected in similarity_bins.items():
        add_panel(name, selected, "nearest_same_target_training_fold_similarity")
...
def run_candidate(
    ...
    root = Path(root_override).resolve() if root_override is not None else Path.cwd().resolve()
    ...
    for target in TARGETS:
        ...
        if target in SPECIAL_TARGETS:
            panel_details: dict[str, dict[str, object]] = {}
            ...
            for name, selected in {
                ...
                detail: dict[str, object] = {
                    ...
                    "delta_r2": 0.0,
                    "source": "official_oof_panel",
                }
                if count >= 5 and float(np.var(rows["target"].to_numpy(float)[selected])) > 1.0e-15:
                    delta = float(
                    ...
                    detail.update({"eligible_for_r2": True, "delta_r2": delta})
                    panel_values.append(delta)
                panel_details[name] = detail
            for scaffold_name in sorted(rows["scaffold"].unique()):
                ...
                if count >= 10 and float(np.var(rows["target"].to_numpy(float)[selected])) > 1.0e-15:
                    delta = float(
                        r2_score(rows["target"].to_numpy(float)[selected], rows["candidate_prediction"].to_numpy(float)[selected])
                        - r2_score(rows["target"].to_numpy(float)[selected], rows["baseline_prediction"].to_numpy(float)[selected])
                    )
                    detail.update({"eligible_for_r2": True, "delta_r2": delta})
                    panel_values.append(delta)
                panel_details[f"scaffold_{scaffold_name}"] = detail
            target_reports[target]["panels"] = panel_details
            target_reports[target]["panel_counts"] = {
                name: int(value["rows"]) for name, value in panel_details.items()
            }
            target_reports[target]["minimum_panel_delta"] = float(min(panel_values)) if panel_values else 0.0
        else:
    ...
    report = {
        "schema_version": "ppp.round2.mixed-seven-target-candidate-run.v2",
        ...
        "source_hashes": source_hashes_override or {
            "candidate_script": source_hash_override or "portable_runtime_source",
            "reference_module": "portable_embedded_source_required",
            "ei_route_module": "portable_embedded_source_required",
            "eea_route_module": "portable_embedded_source_required",
            "metric_plumbing": "portable_embedded_source_required",
        },
    ...
    write_json(run_dir / "metrics.json", {
        "schema_version": "ppp.round2.metrics.v1",
        "experiment_id": run_dir.name,
        "mean_baseline_r2": mean_baseline,
        "mean_candidate_r2": mean_candidate,
        "mean_gain": mean_candidate - mean_baseline,
        "maximum_target_loss": report["maximum_target_loss"],
        "target_loss_gate_pass": report["target_loss_gate_pass"],
        "all_transfer_panels_pass": report["all_transfer_panels_pass"],
        "targets": target_reports,
    })
    (run_dir / "run.log").write_text(
        "stage=clean_official_only\n"
        f"experiment_id={run_dir.name}\n"
        f"rows={len(submission)}\n"
        f"mean_candidate_r2={mean_candidate:.16f}\n"
        f"mean_gain={mean_candidate - mean_baseline:.16f}\n"
    ...
    (run_dir / "decision.md").write_text(
        f"# {run_dir.name}\n\n"
        f"Decision: **candidate_frozen_pending_local_eval**. Mean R2 is `{mean_candidate:.16f}`; "
        f"gain is `{mean_candidate - mean_baseline:.16f}` with no target loss. "
        "The candidate is clean official-only and remains isolated from local_eval and Kaggle actions.\n",
        encoding="utf-8",
    )
    manifest = []
    ...
    manifest.extend([
        f"{report['source_hashes']['candidate_script']}  SOURCE tools/round2_mixed_candidate_v5.py",
        f"{report['source_hashes']['reference_module']}  SOURCE tools/initial_reference_pipeline.py",
        f"{report['source_hashes']['ei_route_module']}  SOURCE tools/round2_ei_scaffold_abstaining_gap_identity_v4_portable.py",
        f"{report['source_hashes']['eea_route_module']}  SOURCE tools/round2_eea_scaffold_abstaining_gap_identity_v7_portable.py",
...
def main() -> None:
    ...
    tool_root = Path.cwd() / "tools"
    source_hashes = {
        "candidate_script": sha256_file(tool_root / "round2_mixed_candidate_v5.py"),
