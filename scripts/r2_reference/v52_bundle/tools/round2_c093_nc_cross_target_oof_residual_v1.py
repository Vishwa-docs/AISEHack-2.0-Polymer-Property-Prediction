...
TARGET = "nc"
AUXILIARY = ("tg", "egc", "egb", "ei", "eea", "eps")
PARENT_WEIGHTS = np.asarray([0.09182193817617626, 0.13716239711054817, 0.3884232411155375, 0.38259242359773804], dtype=np.float64)
PARENT_INTERCEPT = -0.00022872900859654687
...
def parent_arms(dense: np.ndarray, sparse_parts: list[Any], fingerprints: list[Any], y_global: np.ndarray, train: np.ndarray, validation: np.ndarray) -> np.ndarray:
    ...
    if not np.isfinite(values).all():
        raise RuntimeError("non-finite Nc parent arm prediction")
    return values
...
def nested_split(
    ...
    return {
        ...
        "blend_name": "frozen_c050_nc_v7_weights",
...
def main() -> None:
    ...
    for fold in range(5):
        ...
        replay_dense = fold_target_dense(deterministic, cross_values, cross_available, TARGET, key_groups, set(groups[validation]))
        replay_arms = parent_arms(replay_dense, sparse_parts, fingerprints, y_global, indices[training], indices[validation])
        replay_parent = reference.clip_prediction(y[training], replay_arms @ PARENT_WEIGHTS + PARENT_INTERCEPT)
    ...
    # Deployment-matched Nc component: fit the residual model on cross-fitted
    # predictions for every official Nc training row, then predict the 153
    # official test Nc rows.  This remains source-only and does not consume
    ...
    test_parent_arms_replay = parent_arms(test_dense, sparse_parts, fingerprints, y_global, indices, test_global_indices)
    test_parent_replay = reference.clip_prediction(y, test_parent_arms_replay @ PARENT_WEIGHTS + PARENT_INTERCEPT)
    ...
    if len(test_candidate) != 153 or not np.isfinite(test_candidate).all():
        raise RuntimeError("Nc test component output contract failed")
    passed = bool(passed and parent_replay_oof_max_abs <= 1.0e-12 and parent_replay_test_max_abs <= 1.0e-12)
    script_path = root / "tools" / "round2_c093_nc_cross_target_oof_residual_v1.py"
    reference_path = root / "tools" / "initial_reference_pipeline.py"
    audit = {
        "schema_version": "ppp.round2.c093.nc-cross-target-oof-residual-run.v1",
        "experiment_id": run_dir.name,
        "created_at": datetime.now().astimezone().isoformat(),
        "parent": "C050-v7 Nc configuration regenerated with fold-masked grouped parent",
    ...
    write_json(run_dir / "config.json", {
        "schema_version": "ppp.round2.c093.nc-cross-target-oof-residual.v1",
        "seed": 2026,
        "target": TARGET,
        "auxiliary_targets": AUXILIARY,
        "outer_folds": 5,
    ...
    (run_dir / "decision.md").write_text(
        f"# R2-C093 Nc cross-target OOF residual v1\n\nDecision: **{audit['decision']}**. Nc component only; no full candidate or local_eval diagnostic was created by this bounded component run.\n",
        encoding="utf-8",
    )
    ...
    manifest.extend([
        f"{audit['source_hashes']['script']}  SOURCE tools/round2_c093_nc_cross_target_oof_residual_v1.py",
