#!/usr/bin/env python3
"""C324 lightweight no-archive Tg/Egc read-across overlay over C312.
...
It is the over-budget fallback for C323: the same branch/target/data hypothesis,
but with only KNN/Tanimoto, sparse Ridge, and their mean.  It intentionally
omits the dense HGB/ExtraTrees arms that made C323 expensive.
"""
...
def guard_path(path: Path, *, allow_output: bool = False) -> None:
    ...
    if not allow_output:
        if "/archive/" in low or low.endswith("/archive") or "with_archive" in low:
            raise RuntimeError(f"Refusing archive/cross-branch input path for no-archive C324: {path}")


def load_base(path: Path, ids: np.ndarray) -> pd.DataFrame:
    guard_path(path)
    if "without_archive" not in str(path):
        raise RuntimeError(f"C324 base must be branch-local without_archive: {path}")
...
def build_feature_space(train: pd.DataFrame, test: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        ...
        "dense": np.empty((len(keys), 0), dtype=np.float64),
...
def parse_targets(value: str) -> tuple[str, ...]:
    ...
    if invalid:
        raise RuntimeError(f"C324 only supports {ACTIVE_TARGETS}, got {invalid}")
    return targets
...
def main() -> None:
    ...
    if "without_archive" not in str(output):
        raise RuntimeError(f"C324 output must live in without_archive namespace: {output}")
    ...
    record = {
        "schema_version": "ppp.round2.c324.noarchive-tg-egc-light-readacross-overlay.v1",
        ...
        "method": "lightweight support-gated Tg/Egc KNN/Ridge read-across overlay over C312 fallback",
