#!/usr/bin/env python3
"""Official-only Egb Morgan-fragment residual screen."""
...
def main() -> None:
    ...
    if not np.array_equal(frame["canonical"].astype(str).to_numpy(), parent_rows["canonical"].astype(str).to_numpy()):
        raise RuntimeError("exact v7 Egb parent row alignment failed")
    ...
    if not np.array_equal(frame["canonical"].astype(str).to_numpy(), replay_parent_rows["canonical"].astype(str).to_numpy()):
        raise RuntimeError("exact v7 Egb replay row alignment failed")
    ...
    morgan_matrix = reference.morgan_count_matrix(molecules, radius=2, bits=4096)
    morgan_dense = np.asarray(morgan_matrix.toarray(), dtype=np.float64)
    morgan_features = morgan_dense[target_indices]
    morgan_names = [f"morgan_r2_count_{index}" for index in range(morgan_features.shape[1])]
    features = np.hstack([base_features, extra_features, morgan_features])
    feature_names = base_names + extra_names + morgan_names
    ...
    if not np.isfinite(candidate).all():
        raise RuntimeError("non-finite Egb OOF candidate")
    ...
    if len(component) != 224 or component["id"].duplicated().any() or not np.array_equal(component["id"].to_numpy(), test_frame["id"].to_numpy()) or not np.isfinite(component["candidate_prediction"].to_numpy()).all():
        raise RuntimeError("Egb component output contract failed")
    component.to_csv(run_dir / "egb_component_predictions.csv", index=False)
    ...
    gates = {"gain_pass": delta >= 0.01, "fold_pass": sum(row["delta_r2"] > 0 for row in fold_rows) >= 4, "bootstrap_pass": lower > 0.0, "panel_pass": minimum_panel >= 0.0, "component_rows_pass": len(component) == 224, "parent_replay_oof_pass": parent_oof_replay_max_abs <= 1.0e-12, "parent_replay_test_pass": parent_test_replay_max_abs <= 1.0e-12}
    passed = bool(all(gates.values()))
    source_names = ("round2_c072_egb_morgan_fragment_residual.py", "round2_c063_egb_endpoint_conjugation_residual.py", "initial_reference_pipeline.py", "round2_eea_cross_target_oof_residual_stack.py")
    report = {
        "schema_version": "ppp.round2.c072.egb-morgan-fragment-residual.v1",
        "experiment_id": run_dir.name,
        "created_at": datetime.now().astimezone().isoformat(),
        "parent": "R2-C050-20260803-2130-mixed-c001-gap-components-v7; exact v7 Egb parent regenerated twice in-process",
    ...
    write_json(run_dir / "config.json", {"seed": SEED, "target": TARGET, "feature_family": "official-SMILES Morgan radius-2 fragment counts plus deterministic physicochemical descriptors", "model": "Ridge", "ridge_alpha": ALPHA, "residual_weight": RESIDUAL_WEIGHT, "morgan_radius": 2, "morgan_bits": 4096, "outer": "canonical_no_stereo GroupKFold(5)", "bootstrap_resamples": 2000, "no_hyperparameter_sweep": True, "external_label_file_read": False, "local_eval_read": False, "parent_replay_oof_max_abs": parent_oof_replay_max_abs, "parent_replay_test_max_abs": parent_test_replay_max_abs})
