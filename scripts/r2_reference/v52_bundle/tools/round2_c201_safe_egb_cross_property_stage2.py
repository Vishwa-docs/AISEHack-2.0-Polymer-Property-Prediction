#!/usr/bin/env python3
"""C201: safe Egb cross-property Stage-2 residual.

This is a cleaned, bounded version of the Claude Stage-2 idea for the Egb target.
It tests one scientific factor: whether fold-available Egc/Eea partner features
with a structure-only fallback can improve Egb beyond the exact C050 parent.
...
- active target Egb is never used as a feature;
- every validation fold excludes the outer Egb validation no-stereo groups from
...
def fit_active_target(
    ...
    if not np.isfinite(candidate).all():
        raise RuntimeError("C201 produced non-finite Egb OOF predictions")
...
def full_test_predictions(
    ...
    if not np.array_equal(test_rows["id"].to_numpy(int), test_detail["id"].to_numpy(int)):
        raise RuntimeError("C201 Egb test ID alignment failed")
...
def main() -> None:
    ...
    active_candidate, active_report, egc_obs, eea_obs = fit_active_target(parent, dense, sparse_features)
    ...
    if len(predictions) != 4940 or not np.array_equal(predictions["id"].to_numpy(int), np.arange(1, 4941)):
        raise RuntimeError("C201 ID coverage/order contract failed")
    if not np.isfinite(predictions["target"].to_numpy(np.float64)).all():
        raise RuntimeError("C201 produced non-finite full predictions")

    report = {
        "schema_version": "ppp.round2.c201.safe-egb-cross-property-stage2.v1",
