#!/usr/bin/env python3
"""Official-only EPS dielectric/polarizability residual screen."""
...
def main() -> None:
    ...
    if not np.array_equal(frame["canonical"].astype(str).to_numpy(), replay_rows["canonical"].astype(str).to_numpy()):
        raise RuntimeError("exact v7 Ei replay row alignment failed")
    ...
    features = np.hstack([base_features, extra_features])
    ...
    gates = {"gain_pass": delta >= 0.01, "fold_pass": sum(row["delta_r2"] > 0 for row in fold_rows) >= 4, "bootstrap_pass": lower > 0.0, "panel_pass": minimum_panel >= 0.0, "component_rows_pass": len(component) == 148, "parent_replay_oof_pass": parent_oof_replay_max_abs <= 1.0e-12, "parent_replay_test_pass": parent_test_replay_max_abs <= 1.0e-12}
    passed = bool(all(gates.values()))
    source_names = ("round2_c070_ei_smiles_extra_trees_residual.py", "round2_c068_eps_physics_informed_residual.py", "round2_c063_egb_endpoint_conjugation_residual.py", "round2_mixed_candidate_v7.py", "initial_reference_pipeline.py", "round2_eea_cross_target_oof_residual_stack.py")
    report = {
        "schema_version": "ppp.round2.c070.ei-smiles-extra-trees-residual.v1",
        "experiment_id": run_dir.name,
        "created_at": datetime.now().astimezone().isoformat(),
        "parent": "R2-C050-20260803-2130-mixed-c001-gap-components-v7; exact v7 Ei specialized parent regenerated twice in-process",
