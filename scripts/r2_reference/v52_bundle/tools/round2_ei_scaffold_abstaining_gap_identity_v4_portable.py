...
def main() -> None:
    ...
    inputs = {
        name: {**record, "path": str(Path(record["path"]).resolve().relative_to(root))}
    ...
    for target in TARGETS:
        ...
        test_inner_folds = folds_for(groups, 4)
        test_inner_arms = np.full((len(y), 4), np.nan, dtype=np.float64)
        for inner_fold in range(4):
            local_train = np.flatnonzero(test_inner_folds != inner_fold)
            local_validation = np.flatnonzero(test_inner_folds == inner_fold)
            test_inner_arms[local_validation] = reference.predict_base_models(
                target_dense,
                sparse_parts,
                fingerprints,
                y_global,
        ...
        test_weights, test_intercept, _, _ = reference.blend_from_oof(y, test_inner_arms)
        ...
        test_parent = reference.clip_prediction(y, test_arms @ test_weights + test_intercept)
        ...
        test_reconstruction = test_parent.copy()
        ...
        test_prediction_rows.extend(
            {"id": int(row_id), "target": target, "prediction": float(value)}
            for row_id, value in zip(test_frame["id"].to_numpy(int), test_reconstruction, strict=True)
        )

    oof = pd.concat(all_oof, ignore_index=True)
    paired_target = "eea"
    paired_rows = int((oof["target_type"] == paired_target).sum())
    paired_audit = {
        "schema_version": "ppp.round2.paired-target-structural-audit.v1",
        "experiment_id": run_dir.name,
        "component_target": "ei",
        "paired_target": paired_target,
        "modified_targets": ["ei"],
        "oof_target_types": sorted(str(value) for value in oof["target_type"].unique()),
        "paired_prediction_rows": paired_rows,
        "paired_changed_rows": 0,
        "r2_loss": 0.0 if paired_rows == 0 else None,
        "maximum_allowed_loss": 0.003,
        "pass": paired_rows == 0,
        "method": "structural_no_op_verified_by_experiment_emitted_oof_scope",
        "checks": {
            "component_route_changes_ei_only": bool(set(oof["target_type"]) == {"ei"}),
            "no_paired_prediction_rows_emitted": paired_rows == 0,
            "no_paired_rows_changed": paired_rows == 0,
            "candidate_not_created": True,
        },
        "note": "The Ei-only component uses official Eea as an inference covariate but emits no Eea predictions; paired Eea loss is an exact structural zero, not an local_eval-derived score.",
    }
    write_json(run_dir / "paired_target_audit.json", paired_audit)
    for target in TARGETS:
        target_reports[target]["paired_target_gate"] = {
            "target": paired_target,
            "oof_rows": paired_rows,
            "r2_loss": paired_audit["r2_loss"],
            "maximum_allowed_loss": 0.003,
            "pass": paired_audit["pass"],
            "basis": paired_audit["method"],
        }
    pass_targets = [target for target in TARGETS if bool(target_reports[target]["pass"])]
    source_hashes = {
        "script": "portable_embedded_route_source",
    ...
    predictions = pd.DataFrame(test_prediction_rows, columns=["id", "target", "prediction"])
    expected_component_rows = int(test["target_type"].isin(TARGETS).sum())
    if len(predictions) != expected_component_rows or not np.isfinite(predictions["prediction"].to_numpy(float)).all():
        raise RuntimeError("component predictions.csv must contain one finite prediction per official test row for its target")
    ...
    manifest.extend([
        f"{source_hashes['script']}  SOURCE tools/round2_ei_scaffold_abstaining_gap_identity_v4_portable.py",
