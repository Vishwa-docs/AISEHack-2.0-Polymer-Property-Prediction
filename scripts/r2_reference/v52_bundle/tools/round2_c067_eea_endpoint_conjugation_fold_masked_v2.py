...
def main() -> None:
    ...
    # Deployment support is determined from official auxiliary labels available
    # in the training/archive pool; a test group is not required to be present
    # in the Eea target frame itself.
    test_support = np.asarray([
        value in aux_canonical["ei"] and value in aux_canonical["egc"]
        for value in test_canonical
    ], dtype=bool)
    ...
    source_names = (
        "round2_c067_eea_endpoint_conjugation_fold_masked_v2.py",
    ...
    report = {
        "schema_version": "ppp.round2.c067.eea-endpoint-conjugation-fold-masked.v2",
