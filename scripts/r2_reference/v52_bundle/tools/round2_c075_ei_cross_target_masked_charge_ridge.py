#!/usr/bin/env python3
"""Official-only strictly cross-target-masked Ei charge Ridge screen."""
...
def main() -> None:
    ...
    def generate_parent() -> tuple[np.ndarray, np.ndarray]:
        ...
        for fold in range(5):
            ...
            outer_parent, _, _ = strict_specialized_split(
                ...
                dense_base, cross_values, cross_available, key_groups, canonical_groups,
                sparse_parts, fingerprints, target_global_indices, y_global,
        ...
        test_parent = strict_specialized_test(
            ...
            dense_base, cross_values, cross_available, key_groups, canonical_groups,
            sparse_parts, fingerprints, target_global_indices, test_rows, y_global,
            config, gap_table, maps,
        )
    ...
    for fold in range(5):
        ...
        strict_parent, inner_parent, parent_meta = strict_specialized_split(
            y, groups, scaffolds, frame["canonical"].astype(str).to_numpy(), training, validation,
            dense_base, cross_values, cross_available, key_groups, canonical_groups,
            sparse_parts, fingerprints, target_global_indices, y_global, config, gap_table, maps,
        )
    ...
    gates = {"gain_pass": delta >= 0.01, "fold_pass": sum(row["delta_r2"] > 0 for row in fold_rows) >= 4, "bootstrap_pass": lower > 0.0, "panel_pass": minimum_panel >= 0.0, "component_rows_pass": len(component) == 148, "parent_replay_oof_pass": parent_oof_replay_max_abs <= 1.0e-12, "parent_replay_test_pass": parent_test_replay_max_abs <= 1.0e-12, "electronic_feature_support_pass": electronic_train_finite_rows == len(y) and electronic_test_finite_rows == len(test_frame)}
    passed = bool(all(gates.values()))
    source_names = ("round2_c075_ei_cross_target_masked_charge_ridge.py", "round2_c063_egb_endpoint_conjugation_residual.py", "round2_ei_scaffold_abstaining_gap_identity_v4_portable.py", "initial_reference_pipeline.py", "round2_eea_cross_target_oof_residual_stack.py")
    report = {
        "schema_version": "ppp.round2.c075.ei-cross-target-masked-charge-ridge.v1",
        "experiment_id": run_dir.name,
        "created_at": datetime.now().astimezone().isoformat(),
        "parent": "R2-C050-20260803-2130-mixed-c001-gap-components-v7; strict outer/inner parent and fold-masked cross-target covariates/maps",
