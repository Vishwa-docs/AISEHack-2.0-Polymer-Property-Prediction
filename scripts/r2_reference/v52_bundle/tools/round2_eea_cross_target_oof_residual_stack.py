...
def parent_arms(dense: np.ndarray, sparse_parts: list[Any], fingerprints: list[Any], y_global: np.ndarray, train: np.ndarray, validation: np.ndarray) -> np.ndarray:
    ...
    if not np.isfinite(values).all():
        raise RuntimeError("non-finite Eea parent arm prediction")
    return values
...
def nested_split(
    ...
    for fold in range(4):
        ...
        inner_parent_arms[inner_folds == fold] = parent_arms(
            dense, sparse_parts, fingerprints, y_global, global_indices[local_train], global_indices[local_validation]
        )
    ...
    aux_columns = [reference.TARGETS.index(target) for target in AUXILIARY]
    available_inner = cross_available[global_indices[outer_train]][:, aux_columns]
    available_outer = cross_available[global_indices[outer_validation]][:, aux_columns]
    inner_features = np.hstack([inner_parent[:, None], inner_aux, available_inner])
    outer_features = np.hstack([outer_parent[:, None], outer_aux, available_outer])
    ...
    candidate = reference.clip_prediction(y[outer_train], outer_parent + 0.5 * correction)
    return {
        "parent": outer_parent,
        "candidate": candidate,
...
def main() -> None:
    ...
    for fold in range(5):
        ...
        result = nested_split(
            ...
            y_global, indices, cross_available, aux_info, deterministic
        )
    ...
    script_path = root / "tools" / "round2_eea_cross_target_oof_residual_stack.py"
    reference_path = root / "tools" / "initial_reference_pipeline.py"
    audit = {
        "schema_version": "ppp.round2.eea-cross-target-oof-residual-stack-run.v1",
        "experiment_id": run_dir.name,
        "created_at": datetime.now().astimezone().isoformat(),
        "parent": "R2-C030-20260803-2230-eea-low-rank-cross-property-calibration",
    ...
    write_json(run_dir / "config.json", {
        "schema_version": "ppp.round2.eea-cross-target-oof-residual-stack.v1",
        "seed": 2026,
        "target": TARGET,
        "auxiliary_targets": AUXILIARY,
        "outer_folds": 5,
        ...
        "residual_model": "fold-local standardized Ridge(alpha=30), fixed correction strength 0.5",
        "official_inputs": inputs,
    ...
    manifest.extend([
        f"{audit['source_hashes']['script']}  SOURCE tools/round2_eea_cross_target_oof_residual_stack.py",
