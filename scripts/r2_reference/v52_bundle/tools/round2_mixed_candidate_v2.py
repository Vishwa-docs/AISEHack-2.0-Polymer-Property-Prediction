...
def specialized_target(
    ...
    for fold_id in range(5):
        ...
        raw_candidate, route, partner, _ = gap.route_predictions(
            target,
...
def run_candidate(data_dir: str | Path, run_dir: Path, output: Path) -> dict[str, object]:
    ...
    for target in TARGETS:
        ...
        if target in SPECIAL_TARGETS:
            ...
            target_reports[target]["panels"] = {"unchanged_c001_reference": 0.0}
            target_reports[target]["minimum_panel_delta"] = 0.0
    ...
    manifest.extend([
        f"{report['source_hashes']['candidate_script']}  SOURCE tools/round2_mixed_candidate_v2.py",
