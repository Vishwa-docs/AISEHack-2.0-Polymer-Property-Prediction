...
def main() -> None:
    ...
    for target in TARGETS:
        ...
        for name_bin, lower, upper in (("lt_0.30", 0.0, 0.30), ("0.30_0.50", 0.30, 0.50), ("0.50_0.70", 0.50, 0.70), ("ge_0.70", 0.70, 1.01)):
            ...
            report["low_similarity"][name_bin] = {"rows": int(np.sum(selected)), "baseline_r2": baseline_r2}
            for arm, name in enumerate(ARMS):
                report["low_similarity"][name][f"{name}_r2"] = r2(y[selected], candidates[selected, arm])
                report["low_similarity"][name][f"{name}_delta_r2"] = report["low_similarity"][name][f"{name}_r2"] - baseline_r2
    ...
    for target, report in reports.items():
        ...
        low = [value[best][f"{best}_delta_r2"] for value in report["low_similarity"].values()]
