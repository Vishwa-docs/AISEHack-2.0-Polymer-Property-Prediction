#!/usr/bin/env python3
"""C186: explicit polymer chemistry grammar and sparse physical formulas."""
...
from rdkit.Chem import Crippen, Descriptors, EState, Lipinski, rdMolDescriptors
...
COMPILED = {name: Chem.MolFromSmarts(pattern) for name, pattern in PATTERNS.items()}
...
def molecule_features(mol: Chem.Mol) -> np.ndarray:
    ...
    counts = np.array([float(len(mol.GetSubstructMatches(pattern))) if pattern is not None else 0.0 for pattern in COMPILED.values()])
    aromatic = sum(int(atom.GetIsAromatic()) for atom in mol.GetAtoms())
    sp2 = sum(int(atom.GetHybridization() == Chem.HybridizationType.SP2) for atom in mol.GetAtoms())
    sp3 = sum(int(atom.GetHybridization() == Chem.HybridizationType.SP3) for atom in mol.GetAtoms())
    hetero = sum(int(atom.GetAtomicNum() not in (0, 1, 6)) for atom in mol.GetAtoms())
    charges = np.array([safe(atom.GetDoubleProp("_GasteigerCharge")) if atom.HasProp("_GasteigerCharge") else 0.0 for atom in mol.GetAtoms()])
    estate = np.asarray(EState.EStateIndices(mol), dtype=float)
    estate[~np.isfinite(estate)] = 0.0
    base = np.array([
        safe(Descriptors.MolWt(mol)), safe(Crippen.MolLogP(mol)), safe(Crippen.MolMR(mol)),
        safe(rdMolDescriptors.CalcTPSA(mol)), safe(rdMolDescriptors.CalcLabuteASA(mol)),
        safe(rdMolDescriptors.CalcFractionCSP3(mol)), safe(Lipinski.NumHAcceptors(mol)),
        safe(Lipinski.NumHDonors(mol)), safe(Lipinski.NumRotatableBonds(mol)),
        safe(rdMolDescriptors.CalcNumRings(mol)), safe(rdMolDescriptors.CalcNumAromaticRings(mol)),
        safe(rdMolDescriptors.CalcNumAliphaticRings(mol)), safe(rdMolDescriptors.CalcNumSaturatedRings(mol)),
        safe(Lipinski.NumHeteroatoms(mol)), float(aromatic), float(sp2), float(sp3), float(hetero),
        float(charges.sum()), float(np.abs(charges).sum()), float(charges.min(initial=0.0)), float(charges.max(initial=0.0)),
        float(estate.sum()), float(estate.min(initial=0.0)), float(estate.max(initial=0.0)),
        float(mol.GetNumAtoms()), float(mol.GetNumHeavyAtoms()),
    ])
    normalized = np.array([
        base[0] / heavy, base[2] / heavy, base[3] / heavy, base[4] / heavy,
        base[6] / heavy, base[7] / heavy, base[8] / heavy, base[9] / heavy,
        base[10] / heavy, base[11] / heavy, base[12] / heavy, base[13] / heavy,
        aromatic / atom_count, sp2 / atom_count, sp3 / atom_count, hetero / atom_count,
        np.abs(charges).sum() / atom_count, estate.sum() / atom_count,
    ])
    motif_norm = counts / heavy
    interactions = np.array([
        base[2] / max(base[0], 1.0), base[3] / heavy, base[10] / max(base[9], 1.0),
        (sp2 + aromatic) / atom_count, (base[6] + base[7]) / heavy,
        (counts[0] + counts[1]) / heavy, (counts[5] + counts[6]) / heavy,
        (counts[10] + counts[11] + counts[12]) / heavy,
    ])
    return np.concatenate([base, normalized, counts, motif_norm, interactions]).astype(np.float64)


def build_features(molecules: list[Chem.Mol]) -> tuple[np.ndarray, list[str]]:
    for mol in molecules:
        try:
            Chem.rdPartialCharges.ComputeGasteigerCharges(mol)
        except Exception:
            pass
    X = np.vstack([molecule_features(mol) for mol in molecules])
    X[~np.isfinite(X)] = 0.0
    return X, [f"grammar_{i}" for i in range(X.shape[1])]


def make_model(target: str, fold: int):
    if target in ("ei", "eea"):
        return Ridge(alpha=8.0)
    return ExtraTreesRegressor(n_estimators=450, max_features=0.65, min_samples_leaf=4, random_state=SEED + fold, n_jobs=4)
...
def main() -> None:
    ...
    X, feature_names = build_features(parent["molecules"])
    ...
    for target in TARGETS:
        ...
        for fold in range(carrier.N_FOLDS):
            ...
            scaler = StandardScaler().fit(X[indices[tr]])
            model = make_model(target, fold)
            Xtr = scaler.transform(X[indices[tr]]) if target in ("ei", "eea") else X[indices[tr]]
            Xva = scaler.transform(X[indices[va]]) if target in ("ei", "eea") else X[indices[va]]
            model.fit(Xtr, y[tr] - parent_pred[tr])
            residual_oof[va] = model.predict(Xva)
        ...
        threshold = 0.007 if target in ("eps", "nc") else 0.006
        passed = bool(delta >= threshold and positive >= 4 and lower > 0.0)
        target_reports[target] = {"active": True, "parent_r2": float(r2_score(y, parent_pred)), "candidate_r2": float(r2_score(y, candidate)), "delta_r2": delta, "positive_folds": positive, "group_bootstrap_lower": lower, "threshold": threshold, "pass": passed, "folds": fold_rows}
        ...
        if not np.array_equal(test_rows["id"].to_numpy(int), test_detail["id"].to_numpy(int)):
            raise RuntimeError(f"C186 test ID alignment failed for {target}")
        test_indices = np.asarray([parent["key_to_index"][value] for value in test_rows["canonical"]], dtype=np.int64)
        scaler = StandardScaler().fit(X[indices])
        model = make_model(target, SEED)
        Xtr = scaler.transform(X[indices]) if target in ("ei", "eea") else X[indices]
        Xt = scaler.transform(X[test_indices]) if target in ("ei", "eea") else X[test_indices]
        model.fit(Xtr, y - parent_pred)
        test_parts.append(pd.DataFrame({"id": test_rows["id"].astype(int), "target_type": target, "candidate": test_detail["target"].to_numpy(float) + RESIDUAL_WEIGHT * model.predict(Xt)}))
    ...
    if len(predictions) != 4940 or not np.array_equal(predictions["id"].to_numpy(), np.arange(1, 4941)) or not np.isfinite(predictions["target"].to_numpy(float)).all():
        raise RuntimeError("C186 complete output contract failed")
    report = {"schema_version": "ppp.round2.c186.chemistry-grammar-sisso.v1", "experiment_id": run_dir.name, "created_at": pd.Timestamp.now(tz="Asia/Kolkata").isoformat(), "official_inputs": parent["inputs"], "official_only": True, "external_label_file_read": False, "local_eval_read": False, "pretrained_weights": False, "kaggle_compute": False, "kaggle_upload": False, "parent_replay_parity": parity, "feature_report": {"feature_names": feature_names, "feature_shape": [int(x) for x in X.shape], "patterns": PATTERNS, "selection": "fixed preregistered chemistry grammar; no validation/test feature selection"}, "target_reports": target_reports, "banked_targets": banked, "mean_parent_r2": parent_mean, "mean_candidate_r2": candidate_mean, "mean_gain": candidate_mean - parent_mean, "complete_output_rows": int(len(predictions)), "complete_output_order_pass": True, "full_candidate_gate_pass": bool(banked and candidate_mean - parent_mean >= 0.002), "decision": "candidate_pass_pending_clean_reproduction" if banked and candidate_mean - parent_mean >= 0.002 else "rejected_component_or_full_gate", "elapsed_seconds": float(time.time() - started), "source_hashes": {"runner": sha256_file(Path(__file__)), "parent_builder": sha256_file(root / "Polymer Prediction Challenge Round 2/tools/round2_c097_graph_grammar_hgb_full.py"), "carrier": sha256_file(root / "Polymer Prediction Challenge Round 2/tools/round2_c127_round1_carrier_factory.py")}}
    ...
    write_json(run_dir / "config.json", {"schema_version": "ppp.round2.c186.chemistry-grammar-sisso.v1", "seed": SEED, "residual_weight": RESIDUAL_WEIGHT, "active_targets": list(ACTIVE), "local_eval_read": False})
