...
FILES = [
    ...
    ("round2_mixed_candidate_v7", "round2_mixed_candidate_v7.py"),
]
RUNTIME = "experiments/CLEAN_OFFICIAL_ONLY/R2-C050-20260803-2130-mixed-c001-gap-components-notebook-runtime-v8"
...
def main() -> None:
    ...
    code_lines = [
        ...
        "source_hashes = {'candidate_script': source_hashes['round2_mixed_candidate_v7_module'], 'reference_module': source_hashes['initial_reference_pipeline_module'], 'ei_route_module': source_hashes['round2_ei_scaffold_abstaining_gap_identity_v4_portable_module'], 'eea_route_module': source_hashes['round2_eea_scaffold_abstaining_gap_identity_v7_portable_module'], 'metric_plumbing': source_hashes['round2_eea_cross_target_oof_residual_stack_module']}",
        f"runtime = Path.cwd() / {RUNTIME!r}",
        "runtime.mkdir(parents=True, exist_ok=True)",
        "(runtime / 'protocol.json').write_text(json.dumps({'schema_version': 'ppp.round2.notebook-runtime.v4', 'parent': 'R2-C050-20260803-2130-mixed-c001-gap-components-v7', 'execution_mode': 'saved_embedded_notebook'}, sort_keys=True), encoding='utf-8')",
        "output = runtime / 'notebook_predictions.csv'",
        "result = sys.modules['round2_mixed_candidate_v7'].run_candidate(DATA_DIR, runtime, output, root_override=ROOT, source_hashes_override=source_hashes)",
        "shutil.copyfile(output, Path.cwd() / 'submission.csv')",
        "print(json.dumps({'experiment_id': result['experiment_id'], 'rows': result['submission']['rows'], 'mean_candidate_r2': result['mean_candidate_r2'], 'runtime_prediction_path': str(output), 'submission_path': str(Path.cwd() / 'submission.csv')}, sort_keys=True))",
    ]
    notebook = {
        "cells": [
            {
                ...
                "metadata": {},
                "source": [
                    "# Polymer Round 2 mixed candidate v7\n",
    ...
    output = ROOT / "notebooks" / "R2-C050-mixed-candidate-v7.ipynb"
