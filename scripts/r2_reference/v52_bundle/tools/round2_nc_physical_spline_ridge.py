#!/usr/bin/env python3
"""Nc physical-law spline/Ridge specialist with a masked C001 parent."""
...
def selected_physical_features(descriptor, descriptor_names, physical, physical_names):
    descriptor_keep = [
        "MolWt", "ExactMolWt", "MolMR", "MolLogP", "TPSA", "LabuteASA",
    ...
    physical_keep = [
        ...
        "hetero_atom_count", "rotatable_bonds_approx",
    ]
    ...
    if missing:
        raise RuntimeError(f"Nc specialist descriptors missing: {missing}")
    ...
    heavy = physical[:, pi["heavy_atom_count"]]
    length = physical[:, pi["smiles_length"]]
...
def fit_specialist(features, y_train, train_global, validation_global):
    ...
    model.fit(features[train_global], y_train)
...
def nested_split(y, groups, outer_train, outer_validation, dense, sparse_parts, fingerprints, y_global, global_indices, features, config):
    ...
    specialist = fit_specialist(features, y[outer_train], global_indices[outer_train], global_indices[outer_validation])
    return {
        "parent": outer_parent,
...
def main() -> None:
    ...
    audit = {
        "schema_version": "ppp.round2.nc-physical-spline-ridge-run.v1",
        "experiment_id": run_dir.name,
        "created_at": datetime.now().astimezone().isoformat(),
        "parent": "R2-C037-MASKED-NC-PARENT-GENERATED-IN-RUN",
        "official_inputs": inputs,
        "target": TARGET,
        "feature_names": specialist_names,
        "feature_count": len(specialist_names),
        "model": "SimpleImputer(median) -> SplineTransformer(n_knots=5, degree=2, include_bias=false) -> StandardScaler -> Ridge(alpha=30)",
        "baseline_r2_nested_parent": baseline_r2,
        "candidate_r2_spline_ridge": candidate_r2,
        "delta_r2": delta_r2,
    ...
    pd.DataFrame({"canonical_no_stereo_group": groups, "fold": main_folds, "scaffold": scaffolds, "nearest_tanimoto": nearest, "y": y, "nested_parent": baseline, "spline_ridge": candidate}).to_csv(run_dir / "oof.csv", index=False)
    pd.DataFrame(fold_rows).to_csv(run_dir / "folds.csv", index=False)
    write_json(run_dir / "metrics.json", audit)
    write_json(run_dir / "config.json", {"schema_version": "ppp.round2.nc-physical-spline-ridge.v1", "seed": 2026, "outer_folds": 5, "inner_folds": 4, "features": specialist_names, "knots": 5, "degree": 2, "alpha": 30.0, "official_inputs": inputs})
    ...
    manifest.extend([f"{source_hashes['script']}  SOURCE tools/round2_nc_physical_spline_ridge.py", f"{source_hashes['reference_module']}  SOURCE tools/initial_reference_pipeline.py"])
