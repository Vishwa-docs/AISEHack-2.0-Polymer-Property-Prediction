#!/usr/bin/env python3
"""C203: deterministic clean component compound audit v4.

This child does not introduce a model family. It runs after C202 and assembles
only targets whose own completed clean runs passed their preregistered component
gate. The priority order is frozen here before C201/C202 terminal results are
known, so it is not a same-OOF max selector.

The output is local audit evidence only: no local_eval read, no Kaggle compute,
no upload/submission, and no final-notebook action.
"""
...
def metric_passes(metrics: dict[str, Any], target: str) -> tuple[bool, str]:
    ...
    for field in (
        ...
        "minimum_panel_delta",
        "minimum_stratum_delta",
        "minimum_support_stratum_delta",
    ):
...
def main() -> None:
    ...
    if len(test_prediction) != 4940 or not np.array_equal(test_prediction["id"].to_numpy(int), np.arange(1, 4941)):
        raise RuntimeError("C203 ID coverage/order contract failed")
    if not np.isfinite(test_prediction["target"].to_numpy(float)).all():
        raise RuntimeError("C203 produced non-finite predictions")
    ...
    report = {
        "schema_version": "ppp.round2.c203.clean-component-compound-audit.v4",
    ...
    c200.write_json(output_dir / "config.json", {
        "schema_version": "ppp.round2.c203.clean-component-compound-audit.v4",
        "component_priority": COMPONENT_PRIORITY,
