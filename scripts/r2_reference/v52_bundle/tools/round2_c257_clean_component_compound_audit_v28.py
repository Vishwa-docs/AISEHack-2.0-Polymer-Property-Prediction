#!/usr/bin/env python3
"""C257: deterministic compound audit v28 with C256 Egb priority."""
...
import round2_c255_clean_component_compound_audit_v27 as c255


SCHEMA = "ppp.round2.c257.clean-component-compound-audit.v28"
C256_ID = "R2-C256-20260805-0920-egb-current-domain-residual-v1"
CHANGED_FACTOR = (
    "Insert C256 Egb current-domain residual as first Egb priority under the normal component gate while "
    "preserving C255's C254 Tg priority, C253's C252 Nc priority, older fallbacks, and existing EPS/Ei/Eea/Egc priorities."
)
...
def post_patch(run_dir: Path) -> None:
    ...
    metrics["selection_rule"] = (
        "first completed clean-passing target component in frozen priority order; C256 first for Egb, "
        "C254 first for Tg, C252 first for Nc; no local_eval/public feedback; no same-OOF max search"
    )
    source_hashes = dict(metrics.get("source_hashes", {}))
    source_hashes["c255_wrapper_runner"] = h.sha256_file(Path(c255.__file__).resolve())
...
def main() -> None:
    ...
    base.c203.metric_passes = c255.c253.c249.c245.c243.c241.c239.c237.c235.c233.c231.c229.c227.guarded_metric_passes
    base.main()
