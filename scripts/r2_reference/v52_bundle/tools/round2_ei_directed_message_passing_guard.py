...
def fit_graph_model(graph_features: np.ndarray, y: np.ndarray, train_global: np.ndarray, prediction_global: np.ndarray):
    model = make_pipeline(
        StandardScaler(),
        MLPRegressor(
    ...
    model.fit(graph_features[train_global], y[train_global])
...
def main() -> None:
    ...
    for fold in range(5):
        ...
        _, graph_pred = fit_graph_model(graph_features, y, global_indices[training], global_indices[validation])
