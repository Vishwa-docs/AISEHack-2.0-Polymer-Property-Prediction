#!/usr/bin/env python3
"""R3-BB11 [bB11] - Phase B: Selection repair on the clean arm bank. Experiment 16/150. Real grouped-CV pipeline; reads ONLY official Dataset/ inputs."""
...
def run_experiment(output_dir: Path, smoke: bool = False, data_dir: Path | None = None) -> dict[str, object]:
    ...
    print(f"[{EXP_ID}] starting - Selection repair on the clean arm bank")
    def model_fn(X, y):
        arms = [mo.PerTargetEnsemble(seed=SEED + j).fit(X, y) for j in range(3)]
        class _Bag:
