#!/usr/bin/env python3
"""R3-AA01 [aA01] - Phase A: V57 reproduction. Experiment 2/150. Real grouped-CV pipeline; reads ONLY official Dataset/ inputs."""
...
def run_experiment(output_dir: Path, smoke: bool = False, data_dir: Path | None = None) -> dict[str, object]:
    ...
    print(f"[{EXP_ID}] starting - V57 reproduction")
    model_fn = lambda X, y: mo.PerTargetEnsemble(seed=SEED, ridge_alpha=16, et_estimators=140, gbm_rounds=300).fit(X, y)
