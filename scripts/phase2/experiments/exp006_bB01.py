#!/usr/bin/env python3
"""R3-BB01 [bB01] - Phase B: Selection repair on the clean arm bank. Experiment 6/150. Real grouped-CV pipeline; reads ONLY official Dataset/ inputs."""
...
def run_experiment(output_dir: Path, smoke: bool = False, data_dir: Path | None = None) -> dict[str, object]:
    ...
    def model_fn(X, y):
        arms = [mo.PerTargetEnsemble(seed=SEED + j).fit(X, y) for j in range(5)]
        class _Bag:
