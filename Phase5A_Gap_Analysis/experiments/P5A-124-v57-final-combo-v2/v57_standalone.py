...
def run_v57(data_dir, out_path):
    ...
    # ---- P5A-101 arm: tg Tanimoto kNN residual kriging (alpha on C282 OOF) ----
    _t = 'tg'
    ...
    # ---- P5A-102 arm: per-target linear calibration of base on C282 OOF ----
    ...
    # ---- partner machinery (Ridge on char n-grams, KFold OOF, exact train-partner overlay) ----
    ...
    # ---- P5A-103 arm: ei via egc+eea partner identity (imputed: exact labels where available) ----
    _t = 'ei'
    ...
    for _pt in ('egc', 'eea', 'nc'):
        ...
        for _m in range(len(test_df)):
            ...
            if _d and _pt in _d:
                _pred_test[_pt][_m] = _d[_pt]
    # ---- P5A-104 arm: eps via nc^2 + ionic partner (imputed: exact labels where available) ----
    ...
    for _pt in ('egc', 'eea', 'nc'):
        ...
        for _m in range(len(test_df)):
            ...
            if _d and _pt in _d:
                _pred_test[_pt][_m] = _d[_pt]
    # ---- P5A-105 arm: egb via egc covariate ----
    _t = 'egb'
    _idx = np.where(_tt_arr == _t)[0]
    _y = _y_arr[_idx]
    _p = oof['prediction'].to_numpy(float)[_idx]
    ...
    # ---- P5A-109 arm: per-target shrink toward train median (alpha on OOF) ----
    ...
    # ---- P5A-112 arm: tg MAE-optimal residual model (absolute_error HGB on char n-grams) ----
    ...
    # ---- P5A-113 arm: weak-target residual models with train-only random-SMILES augmentation ----
    ...
    # ---- P5A-107 arm: tg residual model on smile_r3-fitted char-SVD features ----
    ...
    # ---- P5A-117 arm: nc-eps exact-partner consistency (nc = sqrt(eps - ionic), eps = nc^2 + ionic) ----
    _eps_d = train_df[train_df['target_type'] == 'eps'][['smiles', 'target']]
    _nc_d = train_df[train_df['target_type'] == 'nc'][['smiles', 'target']]
    _jj = _eps_d.merge(_nc_d, on='smiles', suffixes=('_e', '_n'))
    _ionic_med = float(np.median(_jj['target_e'] - _jj['target_n'] ** 2)) if len(_jj) else 0.767
    ...
    # ---- P5A-118 arm: weak-target MAE-tuned residual models (char Ridge, alpha by OOF MAE) ----
    ...
    # ---- P5A-119 arm: tg GBM residual model (huber HGB on char n-grams) ----
    _t = 'tg'
    _idx = np.where(tr_tt == _t)[0]
    _y = resid[_idx].copy()
    ...
    for _pt in ('egc', 'eea', 'nc'):
        ...
        for _m in range(len(test_df)):
            ...
            if _d and _pt in _d:
                _pred_test[_pt][_m] = _d[_pt]
    # ---- P5A-121 arm: weak-target stacker (Ridge on partner preds + base, OOF alpha) ----
    for _t in ('ei', 'eea', 'nc', 'eps'):
        _idx = np.where(_tt_arr == _t)[0]
        _y = _y_arr[_idx]
        _p = oof['prediction'].to_numpy(float)[_idx]
    ...
    # ---- P5A-122 arm: weak-target Tanimoto KRR residual models ----
    for _t in ('ei', 'eea', 'nc', 'eps'):
        _idx = np.where(tr_tt == _t)[0]
        _y = resid[_idx].copy()
        if len(_y) < 30:
    ...
    # ---- P5A-123 arm: tg char residual model with x8 random-SMILES augmentation ----
