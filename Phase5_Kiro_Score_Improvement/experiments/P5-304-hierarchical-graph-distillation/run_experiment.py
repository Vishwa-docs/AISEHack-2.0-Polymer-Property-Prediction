#!/usr/bin/env python3
"""
Experiment P5-304: Wave 8 (PLAN AMENDMENT) — Hierarchical Cross-Property Residual Graph Distillation
- 3-Tier Multi-Modal Architecture:
  * Tier 1: Dual-corpus SVD (192-dim) + Morgan Fingerprints (2048) + MACCS (166) + Radical geometry.
  * Tier 2: Latent Cross-Property Matrix + VBM/CBM potential coordinate inversion.
  * Tier 3: Graph heat diffusion over nearest neighbors + Conformal boundary shrinkage.
- SLSQP convex optimization across all tiers to maximize validation R2.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-304 Hierarchical Graph Distillation")
    ...
    print("\n=== Stage 2: Training Hierarchical Graph Distillation Ensembles ===", flush=True)
    ...
    for target in TARGETS:
        ...
        models_oof = [oof_lgb, oof_xgb, oof_cat, oof_ridge, oof_et, oof_diff]
        models_test = [test_lgb_folds, test_xgb_folds, test_cat_folds, test_ridge_folds, test_et_folds, test_diff_folds]
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Hierarchical Graph Distillation): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-304 complete! Hash: {pred_hash}", flush=True)
