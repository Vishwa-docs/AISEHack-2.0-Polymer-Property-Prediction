#!/usr/bin/env python3
"""
Experiment P5-301: Wave 7 (PLAN AMENDMENT) — Direct R2 Metric Gradient Optimization
- Implements custom objective surrogates:
  * Weighted variance-normalized residual loss: L = sum((y - y_hat)^2 / Var(y))
  * SLSQP second-order tree ensemble weighting directly maximizing validation R2:
    w* = argmax_{w} R2(y_val, sum(w_i * m_i))
- Coupled with dual-corpus continuous SVD manifolds and physical conservation constraints.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-301 Direct R2 Gradient Optimization")
    ...
    print("\n=== Stage 2: Training GBDTs with Direct R2 Optimization ===", flush=True)
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Direct R2 Optimization): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-301 complete! Hash: {pred_hash}", flush=True)
