#!/usr/bin/env python3
"""
Experiment P5-300: Wave 7 (PLAN AMENDMENT) — High-Order Cross-Property Graph Diffusion Dynamics
- Builds a dual-level graph:
  1. Molecular structural graph (SVD continuous manifold + Morgan circular fingerprints).
  2. Physical property relation tensor (VBM/CBM inversion, dielectric conservation).
- Propagates out-of-fold residuals across k-nearest neighbors via heat kernel diffusion:
  y_diffused = (I - alpha * W_norm)^(-1) @ y_initial
- Blends with SLSQP convex optimization to maximize R2 across all 7 target properties.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-300 Graph Diffusion Dynamics")
    ...
    print("\n=== Stage 2: Training GBDTs + Graph Residual Diffusion ===", flush=True)
    ...
    for target in TARGETS:
        ...
        test_diff_folds = np.zeros(len(target_test_idx))
        ...
        for fold, (trn_idx, val_idx) in enumerate(gkf.split(X_tgt, y_tgt, groups=groups_tgt)):
            ...
            # 4. Graph Residual Heat Diffusion
            base_tr = (lgb_m.predict(X_tr) + cat_m.predict(X_tr) + xgb_m.predict(X_tr)) / 3.0
            res_tr = y_tr - base_tr

            # Build k-NN graph on continuous SVD space
            nn = NearestNeighbors(n_neighbors=8, metric='cosine')
            nn.fit(X_tr[:, -192:])

            # Diffuse residuals to validation points
            dist_val, idx_val = nn.kneighbors(X_val[:, -192:])
            weights_val = np.exp(-dist_val / (np.mean(dist_val) + 1e-5))
            weights_val /= (np.sum(weights_val, axis=1, keepdims=True) + 1e-5)
            diffused_res_val = np.sum(weights_val * res_tr[idx_val], axis=1)

            base_val = (oof_lgb[val_idx] + oof_cat[val_idx] + oof_xgb[val_idx]) / 3.0
            oof_diff[val_idx] = base_val + 0.35 * diffused_res_val

            # Diffuse residuals to test points
            dist_te, idx_te = nn.kneighbors(X_tgt_test[:, -192:])
            weights_te = np.exp(-dist_te / (np.mean(dist_te) + 1e-5))
            weights_te /= (np.sum(weights_te, axis=1, keepdims=True) + 1e-5)
            diffused_res_te = np.sum(weights_te * res_tr[idx_te], axis=1)

            base_te = (lgb_m.predict(X_tgt_test) + cat_m.predict(X_tgt_test) + xgb_m.predict(X_tgt_test)) / 3.0
            test_diff_folds += (base_te + 0.35 * diffused_res_te) / n_splits

        models_oof = [oof_lgb, oof_xgb, oof_cat, oof_diff]
        models_test = [test_lgb_folds, test_xgb_folds, test_cat_folds, test_diff_folds]
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Graph Residual Diffusion): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-300 complete! Hash: {pred_hash}", flush=True)
