#!/usr/bin/env python3
"""
Experiment P5-311: Wave 9 (PLAN AMENDMENT) — Conformal Gaussian Uncertainty-Weighted Target Stacking
- Estimates epistemic and aleatoric prediction uncertainty sigma_i for each polymer across folds.
- Weights training samples inversely by uncertainty: w_i = 1 / (1 + sigma_i / median(sigma)).
- Blends multi-GBDTs with SLSQP convex optimization across all targets.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-311 Conformal Uncertainty Weights")
    ...
    print("\n=== Stage 2: Training GBDTs with Conformal Uncertainty Weights ===", flush=True)
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Conformal Uncertainty Weights): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-311 complete! Hash: {pred_hash}", flush=True)
