#!/usr/bin/env python3
"""
Experiment P5-320: Wave 9 (PLAN AMENDMENT) — Multi-Head Sparse Target Cross-Distillation Ensemble
- Employs teacher-student cross-distillation from high-N models (Tg, Egc) into low-N models (Ei, Eea, eps, nc).
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-320 Sparse Target Cross-Distillation")
    ...
    print("\n=== Stage 2: Training GBDTs with Cross-Distillation ===", flush=True)
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Sparse Target Cross-Distillation): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-320 complete! Hash: {pred_hash}", flush=True)
