#!/usr/bin/env python3
"""
Experiment P5-306: Wave 9 (PLAN AMENDMENT) — Cross-Target Orthogonal Subspace Projection
- Constructs shared polymer backbone manifold via TruncatedSVD (128-dim).
- Projects residual feature components onto target-orthogonal subspaces via Nullspace / Gram-Schmidt projectors:
  P_orth = I - V_shared (V_shared^T V_shared)^(-1) V_shared^T
- Trains specialized LightGBM, XGBoost, CatBoost, Ridge, ExtraTrees on target-specific orthogonal complements.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-306 Orthogonal Subspace Projection")
    ...
    print("\n=== Stage 2: Training Models with Orthogonal Subspace Complements ===", flush=True)
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Orthogonal Subspace Projection): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-306 complete! Hash: {pred_hash}", flush=True)
