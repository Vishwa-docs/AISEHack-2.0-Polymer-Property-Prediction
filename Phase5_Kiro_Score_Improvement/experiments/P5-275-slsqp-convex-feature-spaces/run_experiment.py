#!/usr/bin/env python3
"""
Experiment P5-275: Wave 1 (PLAN AMENDMENT) — SLSQP Convex Blending over Multi-Modal Feature Spaces
- Trains dedicated GBDT & Ridge models on distinct feature projections:
  * Morgan Fingerprints (2048)
  * MACCS Keys (166)
  * Physical & Rotational Descriptors
  * Backbone & Radical Marker Geometry
  * SVD smile_r3 Continuous Representations
- Target-specific constrained SLSQP convex optimization to learn non-negative weights summing to 1.
"""
...
def extract_multi_feature_spaces(smiles_list, smile_r3_sample=None, is_train=True, tfidf_vec=None, svd_model=None):
    print(f"Extracting 10 distinct representation spaces for {len(smiles_list)} polymers...", flush=True)
    mols = [Chem.MolFromSmiles(clean_smiles(s)) for s in smiles_list]
    
    # Space 1: Polymer Topology & Aspect Ratio (5)
    polymer_geom = []
    for s in smiles_list:
        s_raw = str(s)
        n_stars = s_raw.count('*')
        ...
        if m_raw is not None:
            ...
            polymer_geom.append([float(n_stars), backbone_len, float(arom_neighbors), float(aliph_neighbors), mw / max(backbone_len, 1.0)])
        else:
            polymer_geom.append([float(n_stars), float(len(s_raw))/3.0, 0.0, 0.0, 50.0])
    space_geom = np.nan_to_num(np.array(polymer_geom, dtype=np.float32), nan=0.0)

    # Space 2: Morgan Fingerprints (2048)
    morgan_fps = []
    for m in mols:
        if m is not None:
    ...
    space_morgan = np.array(morgan_fps, dtype=np.float32)

    # Space 3: MACCS Keys (166)
    maccs_fps = []
    for m in mols:
        if m is not None:
    ...
    space_maccs = np.array(maccs_fps, dtype=np.float32)

    # Space 4: Physicochemical Descriptors (14)
    desc_matrix = []
    for m in mols:
        if m is not None:
    ...
    space_desc = np.nan_to_num(np.array(desc_matrix, dtype=np.float32), nan=0.0)

    # Space 5: smile_r3 SVD (128)
    ...
    if tfidf_vec is not None and svd_model is not None:
        space_svd = svd_model.transform(tfidf_vec.transform(clean_str)).astype(np.float32)
    else:
        space_svd = np.zeros((len(smiles_list), 128), dtype=np.float32)

    all_feats = np.hstack([space_geom, space_morgan, space_maccs, space_desc, space_svd])
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-275 Multi-Space SLSQP Optimization")
    ...
    X_train_base, tfidf_v, svd_m = extract_multi_feature_spaces(train['smiles'].tolist(), smile_r3_sample=smile_r3_sample, is_train=True)
    X_test_base, _, _ = extract_multi_feature_spaces(test['smiles'].tolist(), is_train=False, tfidf_vec=tfidf_v, svd_model=svd_m)
    ...
    print("\n=== Stage 1: Cross-Property Imputation Matrix ===", flush=True)
    ...
    print("\n=== Stage 2: Target-Specific SLSQP Optimization across 6 Heterogeneous Models ===", flush=True)
    ...
    for target in TARGETS:
        ...
        for fold, (trn_idx, val_idx) in enumerate(gkf.split(X_tgt, y_tgt, groups=groups_tgt)):
            ...
            # 1. LightGBM
            lgb_m = lgb.LGBMRegressor(n_estimators=500, learning_rate=0.03, num_leaves=31, subsample=0.8, colsample_bytree=0.8, random_state=2026 + fold, n_jobs=-1, verbose=-1)
            lgb_m.fit(X_tr, y_tr)
            oof_lgb[val_idx] = lgb_m.predict(X_val)
            test_lgb_folds += lgb_m.predict(X_tgt_test) / n_splits

            # 2. XGBoost
            xgb_m = xgb.XGBRegressor(n_estimators=500, learning_rate=0.03, max_depth=6, subsample=0.8, colsample_bytree=0.8, random_state=2026 + fold, n_jobs=-1, verbosity=0)
            xgb_m.fit(X_tr, y_tr)
            oof_xgb[val_idx] = xgb_m.predict(X_val)
            test_xgb_folds += xgb_m.predict(X_tgt_test) / n_splits

            # 3. CatBoost
            cat_m = CatBoostRegressor(iterations=500, learning_rate=0.035, depth=6, random_seed=2026 + fold, verbose=0)
            cat_m.fit(X_tr, y_tr)
            ...
            # 5. ExtraTrees
            et_m = ExtraTreesRegressor(n_estimators=250, max_depth=16, min_samples_split=4, random_state=2026 + fold, n_jobs=-1)
            et_m.fit(X_tr, y_tr)
            oof_et[val_idx] = et_m.predict(X_val)
            test_et_folds += et_m.predict(X_tgt_test) / n_splits

            # 6. GradientBoostingRegressor
            gbr_m = GradientBoostingRegressor(n_estimators=250, learning_rate=0.035, max_depth=4, subsample=0.8, random_state=2026 + fold)
            gbr_m.fit(X_tr, y_tr)
            oof_gbr[val_idx] = gbr_m.predict(X_val)
            test_gbr_folds += gbr_m.predict(X_tgt_test) / n_splits

        models_oof = [oof_lgb, oof_xgb, oof_cat, oof_ridge, oof_et, oof_gbr]
        models_test = [test_lgb_folds, test_xgb_folds, test_cat_folds, test_ridge_folds, test_et_folds, test_gbr_folds]
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Multi-Space SLSQP Optimization): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-275 complete! Hash: {pred_hash}", flush=True)
