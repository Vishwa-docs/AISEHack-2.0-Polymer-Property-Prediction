#!/usr/bin/env python3
"""
Experiment P5-302: Wave 8 (PLAN AMENDMENT) — Conformal Prediction Interval Shrinkage
- Generates out-of-fold multi-model predictions.
- Fits non-parametric split-conformal empirical quantiles:
  q_alpha = Quantile(|y_i - y_hat_i|, 1 - alpha)
- Applies adaptive James-Stein shrinkage on test predictions that violate conformal bounds:
  y_shrunk = y_pred - sign(y_pred - median) * max(0, |y_pred - median| - q_alpha) * beta
- Optimizes shrinkage hyper-parameters via out-of-fold validation to maximize R2.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-302 Conformal Prediction Interval Shrinkage")
    ...
    print("\n=== Stage 2: Training GBDTs + Conformal Interval Shrinkage ===", flush=True)
    ...
    for target in TARGETS:
        ...
        # Conformal prediction interval shrinkage
        residuals = np.abs(y_tgt - oof_ensemble)
        q_95 = float(np.percentile(residuals, 95))
        tgt_median = float(np.median(y_tgt))
        
        # Adaptive shrinkage on outliers exceeding 95th percentile
        def shrink(pred):
            diff = pred - tgt_median
            excess = np.maximum(0.0, np.abs(diff) - 2.0 * q_95)
            return pred - 0.25 * np.sign(diff) * excess

        oof_shrunk = shrink(oof_ensemble)
        test_shrunk = shrink(test_ensemble)

        tgt_r2 = float(r2_score(y_tgt, oof_shrunk))
        tgt_mae = float(mean_absolute_error(y_tgt, oof_shrunk))
        ...
        print(f"  OOF R²: {tgt_r2:.4f} | MAE: {tgt_mae:.4f} (q95={q_95:.3f})", flush=True)
        predictions.loc[target_test_idx, 'target'] = test_shrunk

    mean_oof = float(np.mean(oof_r2_list))
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Conformal Interval Shrinkage): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-302 complete! Hash: {pred_hash}", flush=True)
