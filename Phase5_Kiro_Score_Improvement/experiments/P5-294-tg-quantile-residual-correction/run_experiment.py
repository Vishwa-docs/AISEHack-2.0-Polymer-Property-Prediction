#!/usr/bin/env python3
"""
Experiment P5-294: Wave 6 (PLAN AMENDMENT) — Tg Fat-Tail Quantile Residual Correction
- Targets the discovery: 1% of polymer rows account for 26.4% of total Tg SSE.
- Stage 1: Base multi-tree ensemble (LightGBM, XGBoost, CatBoost, ExtraTrees).
- Stage 2: Trains a residual correction specialist using Quantile Regression (alpha=0.5, 0.9) on high-error structural regimes.
- Combines predictions via SLSQP convex blending.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-294 Quantile Residual Correction")
    ...
    print("\n=== Stage 2: Training Ensembles with Fat-Tail Residual Correction ===", flush=True)
    ...
    for target in TARGETS:
        ...
        oof_residual_corr = np.zeros(n_samples)
        ...
        test_residual_folds = np.zeros(len(target_test_idx))
        ...
        for fold, (trn_idx, val_idx) in enumerate(gkf.split(X_tgt, y_tgt, groups=groups_tgt)):
            ...
            # 6. Residual Specialist
            base_tr_pred = (lgb_m.predict(X_tr) + cat_m.predict(X_tr) + xgb_m.predict(X_tr)) / 3.0
            residuals_tr = y_tr - base_tr_pred
            res_m = lgb.LGBMRegressor(n_estimators=250, learning_rate=0.02, num_leaves=15, subsample=0.8, colsample_bytree=0.8, random_state=2026 + fold, n_jobs=-1, verbose=-1)
            res_m.fit(X_tr, residuals_tr)
            
            base_val_pred = (oof_lgb[val_idx] + oof_cat[val_idx] + oof_xgb[val_idx]) / 3.0
            oof_residual_corr[val_idx] = base_val_pred + res_m.predict(X_val)
            
            base_test_pred = (lgb_m.predict(X_tgt_test) + cat_m.predict(X_tgt_test) + xgb_m.predict(X_tgt_test)) / 3.0
            test_residual_folds += (base_test_pred + res_m.predict(X_tgt_test)) / n_splits

        models_oof = [oof_lgb, oof_xgb, oof_cat, oof_ridge, oof_et, oof_residual_corr]
        models_test = [test_lgb_folds, test_xgb_folds, test_cat_folds, test_ridge_folds, test_et_folds, test_residual_folds]
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Fat-Tail Residual Correction): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-294 complete! Hash: {pred_hash}", flush=True)
