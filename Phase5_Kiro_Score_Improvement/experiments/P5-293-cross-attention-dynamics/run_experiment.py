#!/usr/bin/env python3
"""
Experiment P5-293: Wave 5 (PLAN AMENDMENT) — Multi-Modal Self-Attention Polymer Topological Ensembling
- Self-attention dot-product mechanism over multi-modal embedding subspaces:
  * Backbone radical geometry subspace
  * Morgan circular fingerprint subspace
  * MACCS structural key subspace
  * Physicochemical descriptor subspace
  * Continuous SVD latent subspace
- Computes attention-weighted composite representations followed by SLSQP convex GBDT ensembling.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-293 Self-Attention Polymer Ensembling")
    ...
    print("\n=== Stage 2: Training Self-Attention Multi-Model Ensembles ===", flush=True)
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Self-Attention Ensembling): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-293 complete! Hash: {pred_hash}", flush=True)
