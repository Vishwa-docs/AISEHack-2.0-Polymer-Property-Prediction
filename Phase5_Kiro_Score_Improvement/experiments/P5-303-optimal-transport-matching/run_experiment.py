#!/usr/bin/env python3
"""
Experiment P5-303: Wave 8 (PLAN AMENDMENT) — Optimal Transport Domain-Shift Alignment
- Computes empirical Wasserstein-like density matching between train and test feature manifolds.
- Re-weights training sample importance via Sinkhorn-style inverse transportation cost:
  w_i = sum_j T_ij where T = argmin <T, C> - lambda * H(T)
- Trains sample-weighted LightGBM, XGBoost, CatBoost, Ridge, ExtraTrees with SLSQP convex optimization.
"""
...
def compute_ot_weights(X_tr, X_te, eps=0.1):
    # Subsample for speed if needed
    X_tr_sub = X_tr[:, -192:]
    X_te_sub = X_te[:, -192:]
    
    # Compute pairwise Euclidean distance matrix
    C = cdist(X_tr_sub, X_te_sub, metric='euclidean')
    # Softmax assignment (Sinkhorn 1-step surrogate)
    K = np.exp(-C / (np.mean(C) * eps + 1e-5))
    weights = np.sum(K, axis=1)
    weights /= (np.mean(weights) + 1e-5)
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-303 Optimal Transport Matching")
    ...
    print("\n=== Stage 2: Training GBDTs with Optimal Transport Sample Weighting ===", flush=True)
    ...
    for target in TARGETS:
        ...
        # Compute sample weights via optimal transport distance
        sample_weights = compute_ot_weights(X_tgt, X_tgt_test)
        ...
        for fold, (trn_idx, val_idx) in enumerate(gkf.split(X_tgt, y_tgt, groups=groups_tgt)):
            X_tr, y_tr, w_tr = X_tgt[trn_idx], y_tgt[trn_idx], sample_weights[trn_idx]
            X_val, y_val = X_tgt[val_idx], y_tgt[val_idx]

            # 1. LightGBM with sample weights
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Optimal Transport Matching): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-303 complete! Hash: {pred_hash}", flush=True)
