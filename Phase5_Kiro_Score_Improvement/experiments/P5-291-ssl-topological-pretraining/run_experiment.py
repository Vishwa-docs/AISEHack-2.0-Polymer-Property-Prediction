#!/usr/bin/env python3
"""
Experiment P5-291: Wave 5 (PLAN AMENDMENT) — Dual-Corpus SSL Topological Continuous Embeddings
- Pre-trains continuous topological representations on official PI1M.csv (polymers) + smile_r3.csv (molecules).
- Builds joint polymer-molecular continuous manifold via multi-scale n-gram SVD + graph invariant hashing.
- Trains cross-property GBDTs with SLSQP convex blending.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-291 Dual-Corpus SSL Continuous Embeddings")
    ...
    print("\n=== Stage 2: Training Cross-Property GBDT Ensembles ===", flush=True)
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Dual-Corpus SSL Continuous Embeddings): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-291 complete! Hash: {pred_hash}", flush=True)
