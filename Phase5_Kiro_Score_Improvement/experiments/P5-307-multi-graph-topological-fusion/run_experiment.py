#!/usr/bin/env python3
"""
Experiment P5-307: Wave 9 (PLAN AMENDMENT) — Multi-Graph Topological Fusion on Radical Invariant Backbones
- Multi-Graph Topological Features:
  1. Radical-to-Radical Path Distance & Conjugation Index
  2. Atom Connectivity Matrices & Balaban / Bertz graph invariants
  3. Continuous SVD (192) on PI1M + smile_r3 combined corpora
    ...
    for s in smiles_list:
        ...
        if m_raw is not None:
            ...
            try:
                ...
                chi0v = float(GraphDescriptors.Chi0v(m_raw))
                chi1v = float(GraphDescriptors.Chi1v(m_raw))
            except Exception:
                balaban_j, bertz_ct, chi0v, chi1v = 0.0, 0.0, 0.0, 0.0

            polymer_geom.append([
                float(n_stars), backbone_len, mw / max(backbone_len, 1.0),
                balaban_j, bertz_ct, chi0v, chi1v
            ])
        else:
            polymer_geom.append([float(n_stars), float(len(s_raw))/3.0, 50.0, 0.0, 0.0, 0.0, 0.0])
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-307 Multi-Graph Topological Fusion")
    ...
    print("\n=== Stage 2: Training Multi-Graph Fused Ensembles ===", flush=True)
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Multi-Graph Topological Fusion): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-307 complete! Hash: {pred_hash}", flush=True)
