#!/usr/bin/env python3
"""
Experiment P5-276: Wave 2 (PLAN AMENDMENT) — Cross-Property 2.0: Iterative Joint Property Matrix Filling
- Mathematical framework:
    Stage 0: Feature extraction (Morgan 2048 + MACCS 166 + RDKit 2D physical descriptors + smile_r3 SVD SSL)
    Stage 1: Primary independent property predictors (Egc, Tg, Nc)
    Stage 2: Iterative joint property imputation (fill full 7xN matrix across all training & test polymers)
    Stage 3: Secondary property refinement with dynamic cross-property conditioning:
        - Ei refined using Egc and Eea
        - Eps refined using Nc (Maxwell constraint)
        - Egb refined using Egc
        - Eea refined using Ei and Egc
    Stage 4: Target-specific SLSQP convex blending across heterogeneous models
"""
...
def slsqp_blend(models_oof, y_true):
    # Optimize weights constrained to sum=1 and w_i >= 0
    k = len(models_oof)
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-276 Iterative Joint Fill")
    ...
    print("Loading datasets for Cross-Property 2.0...", flush=True)
    ...
    print("\n=== Stage 1: Cross-Property 7-Target Joint Imputation Field ===", flush=True)
    # Fit initial base predictors to build the 7-property latent matrix for all polymers
    ...
    for idx, target in enumerate(TARGETS):
        ...
        train_prop_matrix[tgt_train_idx, idx] = oof_pred  # strictly OOF for measured
        test_prop_matrix[:, idx] = test_pred_folds

    # Physical transformations in property space
    # Target index map: 0:tg, 1:egc, 2:egb, 3:ei, 4:eea, 5:eps, 6:nc
    # Physics features:
    # orbital_sum: egc + eea
    # maxwell_eps: nc^2
    phys_train = np.column_stack([
        train_prop_matrix[:, 1] + train_prop_matrix[:, 4], # egc + eea
        train_prop_matrix[:, 6] ** 2,                      # nc^2
        train_prop_matrix[:, 1] - 0.25                     # egb estimate
    ])
    phys_test = np.column_stack([
        test_prop_matrix[:, 1] + test_prop_matrix[:, 4],
        test_prop_matrix[:, 6] ** 2,
        test_prop_matrix[:, 1] - 0.25
    ])

    X_train_full = np.hstack([X_train_base, train_prop_matrix, phys_train])
    X_test_full = np.hstack([X_test_base, test_prop_matrix, phys_test])

    print("\n=== Stage 2: Heterogeneous Ensemble with SLSQP Convex Blending ===", flush=True)
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Cross-Property 2.0 Iterative Fill): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-276 complete! Hash: {pred_hash}", flush=True)
