#!/usr/bin/env python3
"""
Experiment P5-297: Wave 6 (PLAN AMENDMENT) — Staged Residual Cascade PolyEnsemble
- Architecture:
  * Stage 1: Fast linear baseline with Ridge & Huber on joint representation.
  * Stage 2: LightGBM & CatBoost trained on Stage 1 residuals.
  * Stage 3: XGBoost & ExtraTrees trained on Stage 2 residuals.
  * Stage 4: Convex SLSQP multi-stage optimization across all stages.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-297 Staged Residual Cascade")
    ...
    print("\n=== Stage 1: Cross-Property Joint Imputation Matrix ===", flush=True)
    ...
    print("\n=== Stage 2: Training Staged Residual Cascades ===", flush=True)
    ...
    for target in TARGETS:
        ...
        oof_xgb_res = np.zeros(n_samples)
        oof_et_res = np.zeros(n_samples)
        ...
        test_xgb_res_folds = np.zeros(len(target_test_idx))
        test_et_res_folds = np.zeros(len(target_test_idx))
        ...
        for fold, (trn_idx, val_idx) in enumerate(gkf.split(X_tgt, y_tgt, groups=groups_tgt)):
            ...
            # Stage 3: XGBoost on Stage 2 Residuals
            stg2_tr = (lgb_m.predict(X_tr) + cat_m.predict(X_tr)) / 2.0
            res_stg2_tr = y_tr - stg2_tr

            xgb_res_m = xgb.XGBRegressor(n_estimators=300, learning_rate=0.025, max_depth=5, subsample=0.8, colsample_bytree=0.8, random_state=2026 + fold, n_jobs=-1, verbosity=0)
            xgb_res_m.fit(X_tr, res_stg2_tr)
            
            stg2_val = (oof_lgb[val_idx] + oof_cat[val_idx]) / 2.0
            oof_xgb_res[val_idx] = stg2_val + xgb_res_m.predict(X_val)

            stg2_test = (lgb_m.predict(X_tgt_test) + cat_m.predict(X_tgt_test)) / 2.0
            test_xgb_res_folds += (stg2_test + xgb_res_m.predict(X_tgt_test)) / n_splits

            # Stage 4: ExtraTrees on Residuals
            et_res_m = ExtraTreesRegressor(n_estimators=200, max_depth=14, min_samples_split=4, random_state=2026 + fold, n_jobs=-1)
            et_res_m.fit(X_tr, res_stg2_tr)

            oof_et_res[val_idx] = stg2_val + et_res_m.predict(X_val)
            test_et_res_folds += (stg2_test + et_res_m.predict(X_tgt_test)) / n_splits

        models_oof = [oof_ridge, oof_lgb, oof_cat, oof_xgb_res, oof_et_res]
        models_test = [test_ridge_folds, test_lgb_folds, test_cat_folds, test_xgb_res_folds, test_et_res_folds]
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Staged Residual Cascade): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-297 complete! Hash: {pred_hash}", flush=True)
