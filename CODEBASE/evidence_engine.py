...
if __name__ == "__main__":
    import argparse
    p = argparse.ArgumentParser(description="Round-3 evidence engine (explainability / invariance / generalization)")
    p.add_argument("--data-dir", required=True, help="directory with official train.csv / test.csv")
    p.add_argument("--out-dir", default="outputs", help="evidence output directory")
    p.add_argument("--submission", default=None, help="optional submission.csv (id,target) for test-side artifacts")
    args = p.parse_args()
    train, test = load_official(args.data_dir)
    submission = None
    if args.submission and Path(args.submission).exists():
        submission = pd.read_csv(args.submission)
    run_evidence_engine(train, test, submission, args.out_dir)
