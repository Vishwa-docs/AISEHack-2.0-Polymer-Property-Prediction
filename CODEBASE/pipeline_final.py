...
#            4,940-row id,target submission.csv).  Verified answer-panel R2 0.9023.
...
def main(argv=None):
    import argparse as _argparse
    parser = _argparse.ArgumentParser(description="Round-3 final pipeline: V57 submission + explainability/invariance/generalization evidence")
    parser.add_argument('--mode', default='full', choices=['full', 'submission', 'evidence'],
                        help='full = submission + evidence; submission = V57 only; evidence = evidence only')
    parser.add_argument('--data-dir', default=None, help='directory containing official train.csv/test.csv/PI1M.csv')
    parser.add_argument('--out', default='submission.csv', help='output submission path')
    parser.add_argument('--out-dir', default='outputs', help='evidence output directory')
    parser.add_argument('--submission', default=None, help='submission.csv to analyze in evidence mode (defaults to --out)')
    parser.add_argument('--smoke', action='store_true', help='fast demo mode (tiny data)')
    args = parser.parse_args(argv)

    if args.smoke:
        os.environ['PHASE4_SMOKE'] = '1'
    ...
    if data_dir is None:
        ...
        for parent in (here, *here.parents):
            ...
            if (cand / 'train.csv').is_file() and (cand / 'test.csv').is_file():
                ...
                break
        if data_dir is None:
            raise RuntimeError('Could not locate official train/test files; pass --data-dir')

    if args.mode in ('full', 'submission'):
        print('=== [Stage 1/2] V57 submission pipeline ===', flush=True)
        submission = run_v57(data_dir, args.out)
        print('submission written:', args.out, flush=True)
        if args.mode == 'submission':
            print('done (submission only).', flush=True)
            return submission

    # full or evidence: load official data + submission (in-memory for full)
    train, test = load_official(data_dir)
    if args.mode == 'full':
        sub_df = submission  # in-memory from run_v57
    else:
        sub_path = args.submission or args.out
        if not Path(sub_path).exists():
            raise RuntimeError('evidence mode needs a submission CSV: ' + str(sub_path))
        sub_df = pd.read_csv(sub_path)

    print('=== [Stage 2/2] Round-3 evidence suite -> ' + str(args.out_dir) + ' ===', flush=True)
    run_evidence_engine(train, test, sub_df, args.out_dir)
    print('done.', flush=True)
    return sub_df
