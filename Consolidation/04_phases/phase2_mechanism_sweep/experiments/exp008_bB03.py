#!/usr/bin/env python3
"""R3-BB03 [bB03] - Phase B: Selection repair on the clean arm bank. Experiment 8/150. Real grouped-CV pipeline; reads ONLY official Dataset/ inputs."""
...
def run_experiment(output_dir: Path, smoke: bool = False, data_dir: Path | None = None) -> dict[str, object]:
    ...
    model_fn = lambda X, y: mo.FastRidgeArm(alpha=16, seed=SEED).fit(X, y)
    feature_fn = lambda texts: f.full_feature_stack(list(texts), use_svd=True, svd_dim=56)
