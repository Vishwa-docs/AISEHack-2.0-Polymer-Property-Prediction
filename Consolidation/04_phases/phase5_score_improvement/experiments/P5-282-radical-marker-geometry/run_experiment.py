#!/usr/bin/env python3
"""
Experiment P5-282: Wave 3 (PLAN AMENDMENT) — Radical-Marker Geometry & Endpoint Topology
- Specialized polymer repeat-unit features:
    * Number and distance between polymerization attachment points (*)
    * Backbone path length (shortest graph distance between attachment points)
    * Attachment point local chemical environment (aromatic, aliphatic, ester, amide)
    * Repeat-unit aspect ratio and linear density
- smile_r3 SVD continuous embeddings + physical descriptors
- 5-Model Heterogeneous Ensemble with SLSQP convex blending
"""
...
def extract_radical_marker_features(smiles_list, smile_r3_sample=None, is_train=True, tfidf_vec=None, svd_model=None):
    print(f"Extracting radical-marker & endpoint topology features for {len(smiles_list)} polymers...", flush=True)
    mols_clean = [Chem.MolFromSmiles(clean_smiles(s)) for s in smiles_list]
    
    # 1. Radical Marker & Polymer Backbone Geometry
    polymer_geom = []
    for s in smiles_list:
        s_raw = str(s)
        n_stars = s_raw.count('*')
        ...
        if m_raw is not None:
            ...
            # Check neighbors of attachment points
            arom_neighbors = 0
            aliph_neighbors = 0
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-282 Radical Marker Geometry")
    ...
    X_train_full, tfidf_v, svd_m = extract_radical_marker_features(train['smiles'].tolist(), smile_r3_sample=smile_r3_sample, is_train=True)
    X_test_full, _, _ = extract_radical_marker_features(test['smiles'].tolist(), is_train=False, tfidf_vec=tfidf_v, svd_model=svd_m)
    ...
    print("\n=== Training Heterogeneous Ensembles with Radical-Marker Geometry ===", flush=True)
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Radical-Marker Geometry): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-282 complete! Hash: {pred_hash}", flush=True)
