#!/usr/bin/env python3
"""
Experiment P5-292: Wave 5 (PLAN AMENDMENT) — Hierarchical Scaffold & Skeleton Stratified Ensembling
- Murcko scaffold and Bemis-Murcko carbon backbone decomposition.
- Trains specialized hierarchical branches:
  * Aromatic Conjugated Backbone Specialist
  * Aliphatic Flexible Chain Specialist
- Fuses specialized branches with global GBDTs via SLSQP convex blending.
"""
...
def extract_features(smiles_list, smile_r3_sample=None, is_train=True, tfidf_vec=None, svd_model=None):
    print(f"Extracting scaffold & skeleton features for {len(smiles_list)} polymers...", flush=True)
    mols = [Chem.MolFromSmiles(clean_smiles(s)) for s in smiles_list]
    
    # 1. Radical Marker Geometry & Scaffold Properties
    polymer_geom = []
    for s in smiles_list:
        s_raw = str(s)
        n_stars = s_raw.count('*')
        ...
        if m_raw is not None:
            ...
            # Murcko Scaffold
            try:
                scaff = MurckoScaffold.GetScaffoldForMol(m_raw)
                scaff_atoms = scaff.GetNumHeavyAtoms()
                scaff_rings = scaff.GetRingInfo().NumRings()
            except Exception:
                scaff_atoms = 0
                scaff_rings = 0
                
            mw = Descriptors.MolWt(m_raw)
            polymer_geom.append([float(n_stars), backbone_len, float(scaff_atoms), float(scaff_rings), mw / max(backbone_len, 1.0)])
        else:
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-292 Hierarchical Scaffold Ensembling")
    ...
    print("\n=== Stage 1: Cross-Property Ridge Imputation Matrix ===", flush=True)
    ...
    print("\n=== Stage 2: Training Hierarchical Scaffold Ensembles ===", flush=True)
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Hierarchical Scaffold Ensembles): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-292 complete! Hash: {pred_hash}", flush=True)
