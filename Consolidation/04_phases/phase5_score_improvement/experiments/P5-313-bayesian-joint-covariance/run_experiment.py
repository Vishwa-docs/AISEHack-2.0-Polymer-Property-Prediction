#!/usr/bin/env python3
"""
Experiment P5-313: Wave 9 (PLAN AMENDMENT) — Bayesian Joint Covariance Priors for Low-Sample Regimes
- Constructs a Bayesian regularized joint covariance field:
  1. Computes cross-target empirical covariance from overlapping polymers.
  2. Imposes joint shrinkage on Ei, Eea, eps, and nc conditional on Egc and Egb representations.
  3. Blends multi-GBDTs with SLSQP convex optimization across all targets.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-313 Bayesian Joint Covariance Priors")
    ...
    print("\n=== Stage 2: Training GBDTs with Bayesian Joint Covariance Priors ===", flush=True)
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Bayesian Joint Covariance Priors): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-313 complete! Hash: {pred_hash}", flush=True)
