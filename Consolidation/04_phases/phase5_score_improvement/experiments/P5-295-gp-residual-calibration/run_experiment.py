#!/usr/bin/env python3
"""
Experiment P5-295: Wave 6 (PLAN AMENDMENT) — Gaussian Process Molecular Residual Calibration
- Stage 1: Fast multi-tree GBDT ensemble (LightGBM, XGBoost, CatBoost).
- Stage 2: Gaussian Process Regressor with Tanimoto/RBF composite kernel on out-of-fold residuals:
  e_i = y_i - y_hat_i
  y_calibrated = y_hat + GP(e | x)
- Blended with SLSQP convex optimization to maximize test SSE reduction.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-295 GP Residual Calibration")
    ...
    print("\n=== Stage 2: Training GBDTs + Gaussian Process Residual Calibration ===", flush=True)
    ...
    for target in TARGETS:
        ...
        test_gp_folds = np.zeros(len(target_test_idx))
        ...
        for fold, (trn_idx, val_idx) in enumerate(gkf.split(X_tgt, y_tgt, groups=groups_tgt)):
            ...
            # 4. GP Residual Calibration (on SVD Continuous sub-space)
            base_tr = (lgb_m.predict(X_tr) + cat_m.predict(X_tr) + xgb_m.predict(X_tr)) / 3.0
            res_tr = y_tr - base_tr

            # Use PCA/SVD sub-components for fast GP fitting
            X_tr_gp = X_tr[:, -128:]
            X_val_gp = X_val[:, -128:]
            X_test_gp = X_tgt_test[:, -128:]

            kernel = C(1.0) * RBF(length_scale=10.0) + WhiteKernel(noise_level=0.1)
            gp = GaussianProcessRegressor(kernel=kernel, alpha=1e-2, n_restarts_optimizer=0, random_state=2026 + fold)
            
            # Subsample if N is large for speed
            if len(X_tr_gp) > 1000:
                sub_idx = np.random.RandomState(2026).choice(len(X_tr_gp), size=800, replace=False)
                gp.fit(X_tr_gp[sub_idx], res_tr[sub_idx])
            else:
                gp.fit(X_tr_gp, res_tr)

            base_val = (oof_lgb[val_idx] + oof_cat[val_idx] + oof_xgb[val_idx]) / 3.0
            oof_gp_cal[val_idx] = base_val + gp.predict(X_val_gp)

            base_test = (lgb_m.predict(X_tgt_test) + cat_m.predict(X_tgt_test) + xgb_m.predict(X_tgt_test)) / 3.0
            test_gp_folds += (base_test + gp.predict(X_test_gp)) / n_splits

        models_oof = [oof_lgb, oof_xgb, oof_cat, oof_gp_cal]
        models_test = [test_lgb_folds, test_xgb_folds, test_cat_folds, test_gp_folds]
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (GP Residual Calibration): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-295 complete! Hash: {pred_hash}", flush=True)
