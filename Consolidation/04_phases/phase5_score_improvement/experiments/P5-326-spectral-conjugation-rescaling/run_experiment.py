#!/usr/bin/env python3
"""
Experiment P5-326: Wave 10 (PLAN AMENDMENT) — High-Order Cross-Property Conjugation & Spectral Rescaling
- Combines non-linear spectral frequency rescaling with joint electronic conjugate state projections.
- Evaluates multi-GBDTs with SLSQP convex optimization across all targets.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-326 Spectral Conjugation Rescaling")
    ...
    print("\n=== Stage 2: Training GBDTs with Spectral Conjugation Rescaling ===", flush=True)
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Spectral Conjugation Rescaling): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-326 complete! Hash: {pred_hash}", flush=True)
