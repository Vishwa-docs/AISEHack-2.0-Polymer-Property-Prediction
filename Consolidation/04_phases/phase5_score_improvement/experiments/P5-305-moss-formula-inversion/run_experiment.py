#!/usr/bin/env python3
"""
Experiment P5-305: Wave 9 (PLAN AMENDMENT) — Coupled Optical-Dielectric Moss Formula Inversion
- Physical Invariants:
  1. Moss relation: n_c = (95.0 / max(E_gc, 0.5))^(1/4)
  2. Electronic dielectric constant: eps_inf = n_c^2
  3. Total dielectric constant: eps = eps_inf + delta_ionic(TPSA, Pol)
- Enforces non-linear optical feedback iterations over multi-GBDT predictions with SLSQP ensembling.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-305 Moss Formula Inversion")
    ...
    # Physical conservation channels & Moss formula
    # Moss: n_c = (95.0 / E_gc)^(1/4)
    egc_tr_safe = np.maximum(train_prop_matrix[:, 1], 0.5)
    egc_te_safe = np.maximum(test_prop_matrix[:, 1], 0.5)

    nc_moss_tr = (95.0 / egc_tr_safe) ** 0.25
    nc_moss_te = (95.0 / egc_te_safe) ** 0.25

    eps_moss_tr = nc_moss_tr ** 2 + 0.5
    eps_moss_te = nc_moss_te ** 2 + 0.5
    ...
    physics_tr = np.column_stack([train_prop_matrix, ei_cons_tr, egb_cons_tr, nc_moss_tr, eps_moss_tr])
    physics_te = np.column_stack([test_prop_matrix, ei_cons_te, egb_cons_te, nc_moss_te, eps_moss_te])

    X_train_full = np.hstack([X_train_base, physics_tr])
    X_test_full = np.hstack([X_test_base, physics_te])

    print("\n=== Stage 2: Training Coupled Optical-Dielectric Moss Models ===", flush=True)
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Moss Formula Inversion): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-305 complete! Hash: {pred_hash}", flush=True)
