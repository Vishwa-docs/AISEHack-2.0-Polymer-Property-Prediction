#!/usr/bin/env python3
"""
Experiment P5-278: Wave 2 (PLAN AMENDMENT) — Partner Dropout Training for Cross-Property Regularization
- Simulates test-time partner property missingness dynamically.
- During cross-property feature construction, randomly masks out auxiliary property columns (p_drop = 0.5).
- Trains robust multi-model ensemble (LightGBM, XGBoost, CatBoost, ExtraTrees, Ridge) with SLSQP blending.
"""
...
def extract_features(smiles_list, smile_r3_sample=None, is_train=True, tfidf_vec=None, svd_model=None):
    print(f"Extracting representations for {len(smiles_list)} polymers...", flush=True)
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-278 Partner Dropout Experiment")
    ...
    print("\n=== Stage 1: Fitting Cross-Property Ridge Surrogates ===", flush=True)
    ...
    # Apply Partner Dropout to train property matrix during training
    np.random.seed(2026)
    dropout_mask = np.random.binomial(1, p=0.5, size=train_prop_matrix.shape).astype(np.float32)
    train_prop_matrix_dropped = train_prop_matrix * dropout_mask

    X_train_full = np.hstack([X_train_base, train_prop_matrix_dropped])
    X_test_full = np.hstack([X_test_base, test_prop_matrix])

    print("\n=== Stage 2: Training Second-Stage Heterogeneous Ensembles with Partner Dropout ===", flush=True)
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Partner Dropout Regularization): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-278 complete! Hash: {pred_hash}", flush=True)
