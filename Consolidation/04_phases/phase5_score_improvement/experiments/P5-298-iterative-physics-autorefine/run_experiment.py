#!/usr/bin/env python3
"""
Experiment P5-298: Wave 6 (PLAN AMENDMENT) — Physics-Coupled Auto-Refinement Feedback Loop
- Enforces multi-step convergence on fundamental physical relations:
  Step 1: Predict base values for all 7 properties using heterogeneous GBDTs.
  Step 2: Physics conservation adjustment:
          Ei = 0.5 * Ei + 0.5 * (Egc + Eea)
          Egb = 0.5 * Egb + 0.5 * (Egc - 0.25)
          Eps = 0.5 * Eps + 0.5 * (Nc^2 + 0.5)
  Step 3: Train second-pass residual correction models on the physics-adjusted space.
- Final ensemble optimized via SLSQP convex blending.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-298 Physics Auto-Refinement Loop")
    ...
    print("\n=== Stage 1: Initial Cross-Property Field Estimation ===", flush=True)
    ...
    print("\n=== Stage 2: Physics-Coupled Convergence Loop ===", flush=True)
    # Conservation updates
    # TARGETS = ['tg', 'egc', 'egb', 'ei', 'eea', 'eps', 'nc']
    # idx: tg=0, egc=1, egb=2, ei=3, eea=4, eps=5, nc=6
    ei_refined_tr = 0.5 * train_prop_matrix[:, 3] + 0.5 * (train_prop_matrix[:, 1] + train_prop_matrix[:, 4])
    egb_refined_tr = 0.5 * train_prop_matrix[:, 2] + 0.5 * (train_prop_matrix[:, 1] - 0.25)
    eps_refined_tr = 0.5 * train_prop_matrix[:, 5] + 0.5 * (train_prop_matrix[:, 6]**2 + 0.5)

    ei_refined_te = 0.5 * test_prop_matrix[:, 3] + 0.5 * (test_prop_matrix[:, 1] + test_prop_matrix[:, 4])
    egb_refined_te = 0.5 * test_prop_matrix[:, 2] + 0.5 * (test_prop_matrix[:, 1] - 0.25)
    eps_refined_te = 0.5 * test_prop_matrix[:, 5] + 0.5 * (test_prop_matrix[:, 6]**2 + 0.5)

    physics_tr = np.column_stack([train_prop_matrix, ei_refined_tr, egb_refined_tr, eps_refined_tr])
    physics_te = np.column_stack([test_prop_matrix, ei_refined_te, egb_refined_te, eps_refined_te])

    X_train_full = np.hstack([X_train_base, physics_tr])
    X_test_full = np.hstack([X_test_base, physics_te])

    print("\n=== Stage 3: Second-Pass Ensemble Training on Physics Refined Space ===", flush=True)
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Physics Auto-Refinement Feedback): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-298 complete! Hash: {pred_hash}", flush=True)
