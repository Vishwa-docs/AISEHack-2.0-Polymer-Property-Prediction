#!/usr/bin/env python3
"""
Experiment P5-309: Wave 9 (PLAN AMENDMENT) — Polymer Backbone Conjugation Energy Potential Fields
- Features:
  1. Pi-electron conjugation fraction: N_aromatic_atoms / N_backbone_atoms
  2. Delocalization potential field: Conjugation * TPSA / MW
  3. Continuous SVD (192) on PI1M + smile_r3 combined corpora
- Evaluates LightGBM, XGBoost, CatBoost, Ridge, ExtraTrees with SLSQP convex optimization.
"""
...
def extract_features(smiles_list, pi1m_sample=None, smile_r3_sample=None, is_train=True, tfidf_vec=None, svd_model=None):
    ...
    # 1. Radical Marker Geometry & Conjugation Fields
    polymer_geom = []
    for s in smiles_list:
        s_raw = str(s)
        n_stars = s_raw.count('*')
        ...
        if m_raw is not None:
            ...
            n_arom_atoms = sum(1 for a in m_raw.GetAtoms() if a.GetIsAromatic())
            conj_ratio = float(n_arom_atoms) / max(backbone_len, 1.0)
            tpsa = Descriptors.TPSA(m_raw)
            deloc_field = conj_ratio * tpsa / (mw + 1e-5)

            polymer_geom.append([float(n_stars), backbone_len, mw / max(backbone_len, 1.0), conj_ratio, deloc_field])
        else:
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-309 Conjugation Potential Fields")
    ...
    print("\n=== Stage 2: Training Conjugation Potential Field Models ===", flush=True)
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Conjugation Potential Fields): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-309 complete! Hash: {pred_hash}", flush=True)
