#!/usr/bin/env python3
"""
Experiment P5-310: Wave 9 (PLAN AMENDMENT) — Multi-Scale Scaffold Stratified Spline Stacking
- Extracts Murcko scaffolds and clusters polymers into local structural manifolds.
- Fits piecewise linear / spline corrections on out-of-fold predictions within scaffold clusters:
  y_spline = y_pred + f_cluster(y_pred)
- Blends multi-GBDTs + Spline corrections with SLSQP convex optimization across all targets.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-310 Scaffold Spline Stacking")
    ...
    print("\n=== Stage 2: Training GBDTs with Scaffold Spline Stacking ===", flush=True)
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Scaffold Spline Stacking): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-310 complete! Hash: {pred_hash}", flush=True)
