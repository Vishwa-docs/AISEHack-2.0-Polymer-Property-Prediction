#!/usr/bin/env python3
"""
Experiment P5-296: Wave 6 (PLAN AMENDMENT) — VBM / CBM Latent Coordinate Physics Model
- Direct inference of electronic band levels:
    * E_VBM (Valence Band Maximum)
    * E_CBM (Conduction Band Minimum)
    * Physical conservation: E_i = -E_VBM, E_ea = -E_CBM, E_gc = E_CBM - E_VBM = E_i - E_ea
- Optical-Dielectric Polarizability Latent Model:
    * Electronic polarizability (alpha_e) + Ionic polarizability (alpha_i)
    * nc = sqrt(1 + alpha_e)
    * eps = 1 + alpha_e + alpha_i = nc^2 + alpha_i
...
def extract_features(smiles_list, smile_r3_sample=None, is_train=True, tfidf_vec=None, svd_model=None):
    print(f"Extracting multi-modal electronic & physical features for {len(smiles_list)} polymers...", flush=True)
    mols = [Chem.MolFromSmiles(clean_smiles(s)) for s in smiles_list]
    
    # 1. Radical Marker & Backbone Geometry
    polymer_geom = []
    for s in smiles_list:
        s_raw = str(s)
        n_stars = s_raw.count('*')
        ...
        if m_raw is not None:
            ...
            density_proxy = mw / max(backbone_len, 1.0)
            polymer_geom.append([float(n_stars), backbone_len, float(arom_neighbors), float(aliph_neighbors), density_proxy])
        else:
            polymer_geom.append([float(n_stars), float(len(s_raw))/3.0, 0.0, 0.0, 50.0])
    ...
    # 4. Physical & Electronic Descriptors
    desc_matrix = []
    for m in mols:
        if m is not None:
            ...
            vals = [
                mw, logp, tpsa, rot_bonds, hbd, hba, n_arom, n_heavy, mr, val_elec,
                pol_proxy, ionic_proxy, conj_index
            ]
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-296 VBM/CBM Physics Model")
    ...
    print("\n=== Stage 1: Fitting Latent Band Alignment Potentials (VBM / CBM / Polarizability) ===", flush=True)
    # 1. Fit Egc (Bandgap = CBM - VBM)
    egc_idx = train[train['target_type'] == 'egc'].index.to_numpy()
    egc_y = train.loc[egc_idx, 'target'].to_numpy(dtype=np.float32)
    egc_groups = train.loc[egc_idx, 'canon_smiles'].to_numpy()
    
    egc_oof = np.zeros(len(egc_y))
    egc_test_pred = np.zeros(len(test))
    egc_all_train = np.zeros(len(train))

    for fold, (trn_idx, val_idx) in enumerate(gkf.split(X_train_base[egc_idx], egc_y, groups=egc_groups)):
        m = Ridge(alpha=15.0, random_state=2026 + fold)
        m.fit(X_train_base[egc_idx[trn_idx]], egc_y[trn_idx])
        egc_oof[val_idx] = m.predict(X_train_base[egc_idx[val_idx]])
        egc_test_pred += m.predict(X_test_base) / n_splits
        egc_all_train += m.predict(X_train_base) / n_splits

    # 2. Fit Nc (Refractive Index -> Electronic Polarizability alpha_e = nc^2 - 1)
    nc_idx = train[train['target_type'] == 'nc'].index.to_numpy()
    nc_y = train.loc[nc_idx, 'target'].to_numpy(dtype=np.float32)
    nc_groups = train.loc[nc_idx, 'canon_smiles'].to_numpy()

    nc_oof = np.zeros(len(nc_y))
    nc_test_pred = np.zeros(len(test))
    nc_all_train = np.zeros(len(train))

    for fold, (trn_idx, val_idx) in enumerate(gkf.split(X_train_base[nc_idx], nc_y, groups=nc_groups)):
        m = Ridge(alpha=15.0, random_state=2026 + fold)
        m.fit(X_train_base[nc_idx[trn_idx]], nc_y[trn_idx])
        nc_oof[val_idx] = m.predict(X_train_base[nc_idx[val_idx]])
        nc_test_pred += m.predict(X_test_base) / n_splits
        nc_all_train += m.predict(X_train_base) / n_splits

    # Latent coordinates
    # VBM proxy: -(Egc * 0.65 + 3.5)
    # CBM proxy: VBM + Egc
    vbm_train = -(egc_all_train * 0.65 + 3.5)
    cbm_train = vbm_train + egc_all_train
    alpha_e_train = nc_all_train ** 2 - 1.0

    vbm_test = -(egc_test_pred * 0.65 + 3.5)
    cbm_test = vbm_test + egc_test_pred
    alpha_e_test = nc_test_pred ** 2 - 1.0

    latent_train = np.column_stack([egc_all_train, nc_all_train, vbm_train, cbm_train, alpha_e_train, -vbm_train, -cbm_train])
    latent_test = np.column_stack([egc_test_pred, nc_test_pred, vbm_test, cbm_test, alpha_e_test, -vbm_test, -cbm_test])

    X_train_full = np.hstack([X_train_base, latent_train])
    X_test_full = np.hstack([X_test_base, latent_test])

    print("\n=== Stage 2: Training Second-Stage Heterogeneous Ensembles with VBM/CBM Coordinates ===", flush=True)
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (VBM/CBM Physics Latent Model): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-296 complete! Hash: {pred_hash}", flush=True)
