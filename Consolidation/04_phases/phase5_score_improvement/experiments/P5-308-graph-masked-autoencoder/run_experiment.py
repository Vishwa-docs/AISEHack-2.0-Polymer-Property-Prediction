#!/usr/bin/env python3
"""
Experiment P5-308: Wave 9 (PLAN AMENDMENT) — Graph Masked Substructure Autoencoder Pretraining
- Masked N-Gram and Substructure Reconstruction:
  1. Mask 20% of SMILES tokens across 60,000 polymers from PI1M + smile_r3.
  2. Learn 192-dim latent embedding via TruncatedSVD on reconstructed masked n-grams.
  3. Fuses with Morgan (2048), MACCS (166), Graph Invariants, and physical conservation channels.
- Evaluates GBDT ensembles with SLSQP convex optimization across all targets.
"""
...
from rdkit.Chem import AllChem, MACCSkeys, Descriptors, Crippen, rdmolops, GraphDescriptors
...
def extract_features(smiles_list, pi1m_sample=None, smile_r3_sample=None, is_train=True, tfidf_vec=None, svd_model=None):
    ...
    # 1. Radical Marker Geometry & Graph Invariants
    polymer_geom = []
    for s in smiles_list:
        s_raw = str(s)
        n_stars = s_raw.count('*')
        ...
        if m_raw is not None:
            ...
            try:
                balaban_j = float(GraphDescriptors.BalabanJ(m_raw))
                bertz_ct = float(GraphDescriptors.BertzCT(m_raw))
            except Exception:
                balaban_j, bertz_ct = 0.0, 0.0

            polymer_geom.append([float(n_stars), backbone_len, mw / max(backbone_len, 1.0), balaban_j, bertz_ct])
        else:
            polymer_geom.append([float(n_stars), float(len(s_raw))/3.0, 50.0, 0.0, 0.0])
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-308 Graph Masked Autoencoder Pretraining")
    ...
    print("\n=== Stage 2: Training GBDTs with Graph Masked Ensembles ===", flush=True)
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Graph Masked Autoencoder): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-308 complete! Hash: {pred_hash}", flush=True)
