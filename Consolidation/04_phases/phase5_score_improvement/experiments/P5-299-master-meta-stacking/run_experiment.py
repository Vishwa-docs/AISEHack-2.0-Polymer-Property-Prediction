#!/usr/bin/env python3
"""
Experiment P5-299: Wave 6 (PLAN AMENDMENT) — Master Meta-Stacked Specialist Architecture
- Unifies all empirically validated top specialist components:
  * Tg: Extreme Fat-Tail Quantile Residual Ensembles + Radical Topology
  * Egc: Multi-Space SLSQP Convex GBDTs
  * Egb: Bulk Bandgap Multi-Task Joint Optimization
  * Ei: Valence Band Potential + Latent Coordinate Imputation
  * Eea: Electron Affinity SVD + MACCS Diverse Blend
  * Eps: Ionic/Electronic Dielectric Conservation Decomposition
  * Nc: Refractive Index Polarizability + SVD Auto-Refinement
- Stacks all 7 target models with cross-validated out-of-fold blending and SLSQP convex optimization.
"""
...
def extract_features(smiles_list, pi1m_sample=None, smile_r3_sample=None, is_train=True, tfidf_vec=None, svd_model=None):
    print(f"Extracting multi-modal master features for {len(smiles_list)} polymers...", flush=True)
    ...
    # 5. Continuous SVD (128)
    clean_str = [str(s) for s in smiles_list]
    if is_train and smile_r3_sample is not None:
        tfidf_vec = TfidfVectorizer(analyzer='char', ngram_range=(2, 6), max_features=12000, min_df=3)
...
def main():
    parser = argparse.ArgumentParser(description="Run P5-299 Master Meta-Stacking Architecture")
    ...
    n_sample = 10000 if args.smoke else 75000
    ...
    # TARGETS = ['tg', 'egc', 'egb', 'ei', 'eea', 'eps', 'nc']
    ei_cons_tr = train_prop_matrix[:, 1] + train_prop_matrix[:, 4]  # Egc + Eea
    egb_cons_tr = train_prop_matrix[:, 1] - 0.25                    # Egc - 0.25
    eps_cons_tr = train_prop_matrix[:, 6]**2 + 0.5                  # Nc^2 + 0.5
    ...
    print("\n=== Stage 2: Training Meta-Stacked Specialists across All 7 Properties ===", flush=True)
    ...
    for target in TARGETS:
        ...
        test_res_folds = np.zeros(len(target_test_idx))
        ...
        for fold, (trn_idx, val_idx) in enumerate(gkf.split(X_tgt, y_tgt, groups=groups_tgt)):
            ...
            # 1. LightGBM
            lgb_m = lgb.LGBMRegressor(n_estimators=500, learning_rate=0.025, num_leaves=31, subsample=0.8, colsample_bytree=0.8, random_state=2026 + fold, n_jobs=-1, verbose=-1)
            lgb_m.fit(X_tr, y_tr)
            oof_lgb[val_idx] = lgb_m.predict(X_val)
            test_lgb_folds += lgb_m.predict(X_tgt_test) / n_splits

            # 2. XGBoost
            xgb_m = xgb.XGBRegressor(n_estimators=500, learning_rate=0.025, max_depth=6, subsample=0.8, colsample_bytree=0.8, random_state=2026 + fold, n_jobs=-1, verbosity=0)
            xgb_m.fit(X_tr, y_tr)
            oof_xgb[val_idx] = xgb_m.predict(X_val)
            test_xgb_folds += xgb_m.predict(X_tgt_test) / n_splits

            # 3. CatBoost
            cat_m = CatBoostRegressor(iterations=500, learning_rate=0.03, depth=6, random_seed=2026 + fold, verbose=0)
            cat_m.fit(X_tr, y_tr)
            ...
            # 6. Residual Auto-Refinement Learner
            base_tr = (lgb_m.predict(X_tr) + cat_m.predict(X_tr) + xgb_m.predict(X_tr)) / 3.0
            res_tr = y_tr - base_tr
            res_m = lgb.LGBMRegressor(n_estimators=300, learning_rate=0.02, num_leaves=15, subsample=0.8, colsample_bytree=0.8, random_state=2026 + fold, n_jobs=-1, verbose=-1)
            res_m.fit(X_tr, res_tr)

            base_val = (oof_lgb[val_idx] + oof_cat[val_idx] + oof_xgb[val_idx]) / 3.0
            oof_res[val_idx] = base_val + res_m.predict(X_val)

            base_test = (lgb_m.predict(X_tgt_test) + cat_m.predict(X_tgt_test) + xgb_m.predict(X_tgt_test)) / 3.0
            test_res_folds += (base_test + res_m.predict(X_tgt_test)) / n_splits

        models_oof = [oof_lgb, oof_xgb, oof_cat, oof_ridge, oof_et, oof_res]
        models_test = [test_lgb_folds, test_xgb_folds, test_cat_folds, test_ridge_folds, test_et_folds, test_res_folds]
    ...
    print(f"\n==========================================", flush=True)
    print(f"Mean OOF R² (Master Meta-Stacking Architecture): {mean_oof:.5f}", flush=True)
    print(f"==========================================", flush=True)
    ...
    print(f"\n✅ Experiment P5-299 complete! Hash: {pred_hash}", flush=True)
