#!/usr/bin/env python3
"""C122: extend the paired EPS/Nc residual to missing-counterpart rows."""
...
def fit_ridge(x_train: np.ndarray, residual: np.ndarray, x_pred: np.ndarray) -> np.ndarray:
...
def main() -> None:
    ...
    if {path.name for path in run_dir.iterdir()} != {"protocol.json"}:
        raise RuntimeError("C122 requires a fresh protocol-only run directory")
    started = time.time()
    ...
    for target in TARGETS:
        ...
        for fold, (outer_train, outer_valid) in enumerate(outer.split(np.arange(len(y)))):
            supported_prediction = fit_ridge(matrix[train_rows[outer_train]], residual[outer_train], matrix[train_rows[outer_valid]])
            missing_prediction = fit_ridge(structure[train_rows[outer_train]], residual[outer_train], structure[train_rows[outer_valid]])
            supported = np.isfinite(pair_train[outer_valid])
            candidate[outer_valid] = parent[outer_valid]
            candidate[outer_valid[supported]] += RESIDUAL_WEIGHT * supported_prediction[supported]
            candidate[outer_valid[~supported]] += RESIDUAL_WEIGHT * missing_prediction[~supported]
            ...
            fold_rows.append({"fold": fold, "rows": int(len(outer_valid)), "parent_r2": c120.score(y[outer_valid], parent[outer_valid]), "candidate_r2": c120.score(y[outer_valid], candidate[outer_valid]), "delta_r2": c120.score(y[outer_valid], candidate[outer_valid]) - c120.score(y[outer_valid], parent[outer_valid]), "supported_rows": int(np.sum(supported)), "missing_rows": int(np.sum(~supported))})
