#!/usr/bin/env python3
"""Official-only fully nested Ei own-availability charge Ridge screen."""
...
def model() -> object:
    return make_pipeline(SimpleImputer(strategy="median", keep_empty_features=True), StandardScaler(), Ridge(alpha=ALPHA))
...
def main() -> None:
    ...
    full_gap_table = ei_route.make_gap_table(pooled)
    maps = {
    ...
    test_support_keys = {
        value for value in test_canonical
    ...
    electronic_finite_train_rows = int(np.sum(np.all(np.isfinite(electronic[rows]), axis=1)))
    electronic_finite_test_rows = int(np.sum(np.all(np.isfinite(electronic[test_rows]), axis=1)))
    gates = {"gain_pass": delta >= 0.01, "fold_pass": sum(row["delta_r2"] > 0 for row in fold_rows) >= 4, "bootstrap_pass": lower > 0.0, "panel_pass": minimum_panel is not None and minimum_panel >= 0.0, "component_rows_pass": len(component) == 148, "parent_replay_oof_pass": parent_oof_replay_max_abs <= 1.0e-12, "parent_replay_test_pass": parent_test_replay_max_abs <= 1.0e-12, "electronic_feature_support_pass": electronic_finite_train_rows == len(y) and electronic_finite_test_rows == len(test_frame), "own_support_accounted": int(np.sum(supported_oof)) >= 5}
    passed = bool(all(gates.values()))
    source_names = ("round2_c079_ei_own_availability_fully_nested_ridge.py", "round2_c078_ei_fully_nested_availability_charge_ridge.py", "round2_c074_ei_nested_charge_residual.py", "round2_c063_egb_endpoint_conjugation_residual.py", "round2_ei_scaffold_abstaining_gap_identity_v4_portable.py", "initial_reference_pipeline.py", "round2_eea_cross_target_oof_residual_stack.py")
    report = {
        "schema_version": "ppp.round2.c079.ei-own-availability-fully-nested-ridge.v1",
        "experiment_id": run_dir.name,
        "created_at": datetime.now().astimezone().isoformat(),
        "parent": "R2-C050-20260803-2130-mixed-c001-gap-components-v7; fully nested Ei parent/blend/gap regenerated twice with row-local official Eea/Egc availability",
        ...
        "electronic_feature_finite_union_rows": electronic_finite_rows,
        "own_availability_oof_rows": int(np.sum(supported_oof)),
        "deployment_supported_test_rows": int(np.sum(supported_test)),
        "electronic_feature_finite_train_rows": electronic_finite_train_rows,
        "electronic_feature_finite_test_rows": electronic_finite_test_rows,
    ...
    write_json(run_dir / "config.json", {"seed": SEED, "target": TARGET, "feature_family": "official-SMILES endpoint/conjugation/dielectric density plus dummy-capped Gasteiger charge and charge-distance descriptors", "model": "Ridge", "ridge_alpha": ALPHA, "residual_weight": RESIDUAL_WEIGHT, "outer": "canonical_no_stereo GroupKFold(5) with outer-training-only parent/blend/gap labels and row-local official Eea/Egc availability", "bootstrap_resamples": 2000, "no_hyperparameter_sweep": True, "external_label_file_read": False, "local_eval_read": False, "parent_replay_oof_max_abs": parent_oof_replay_max_abs, "parent_replay_test_max_abs": parent_test_replay_max_abs, "electronic_feature_finite_train_rows": electronic_finite_train_rows, "electronic_feature_finite_test_rows": electronic_finite_test_rows, "own_availability_oof_rows": int(np.sum(supported_oof)), "deployment_supported_test_rows": int(np.sum(supported_test))})
    (run_dir / "environment.txt").write_text(f"python={platform.python_version()}\nnumpy={np.__version__}\npandas={pd.__version__}\nrdkit={Chem.rdBase.rdkitVersion}\nplatform={platform.platform()}\n", encoding="utf-8")
    (run_dir / "command.txt").write_text(" ".join(sys.argv) + "\n", encoding="utf-8")
    (run_dir / "decision.md").write_text(f"# {run_dir.name}\n\nDecision: **{report['decision']}**. Ei own-availability fully nested parent R2 `{parent_r2:.12f}`, candidate R2 `{candidate_r2:.12f}`, delta `{delta:+.12f}`. Official-only; no local_eval, external_label file, or Kaggle action.\n", encoding="utf-8")
    manifest = []
