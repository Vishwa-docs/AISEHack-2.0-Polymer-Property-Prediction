#!/usr/bin/env python3
"""Official-only Nc paired-charge/polarizability residual screen."""
...
def main() -> None:
    ...
    parent_oof_replay = parent_oof_replay_frame[parent_oof_replay_frame["target_type"] == TARGET].reset_index(drop=True)["prediction"].to_numpy(float)
    ...
    paired_values = cross_values[target_key_indices, reference.TARGETS.index("eps")]
    paired_available = cross_available[target_key_indices, reference.TARGETS.index("eps")]
    ...
    paired_names = ["official_eps_value", "official_eps_available", "official_eps_value_squared"]
    ...
    for fold in range(5):
        ...
        candidate[validation] = parent_oof[validation]
    ...
    test_parent_replay = detail_replay[detail_replay["target_type"] == TARGET].sort_values("id")["model_prediction"].to_numpy(float)
    parent_test_replay_max_abs = float(np.max(np.abs(test_parent - test_parent_replay)))
    ...
    panels["official_eps_available"] = {"rows": int(np.sum(supported)), "delta_r2": support_delta, "status": "evaluable" if support_delta is not None else "insufficient"}
    panels["official_eps_missing"] = {"rows": int(np.sum(missing)), "delta_r2": missing_delta, "status": "unchanged_control"}
    ...
    source_names = ("round2_c077_nc_paired_charge_polarizability_residual.py", "round2_c076_eps_paired_charge_polarizability_residual.py", "round2_c068_eps_physics_informed_residual.py", "initial_reference_pipeline.py", "round2_eea_cross_target_oof_residual_stack.py")
    report = {
        "schema_version": "ppp.round2.c077.nc-eps-availability-paired-head.v1",
    ...
    write_json(run_dir / "config.json", {"seed": SEED, "target": TARGET, "paired_target": "eps", "feature_family": "official-SMILES dielectric/polarizability densities, dummy-capped Gasteiger charge, fixed endpoint/conjugation descriptors, and exact official EPS value/availability with parent fallback", "residual_weight": RESIDUAL_WEIGHT, "ridge_alpha": ALPHA, "outer": "canonical_no_stereo GroupKFold(5)", "bootstrap_resamples": 2000, "no_hyperparameter_sweep": True, "external_label_file_read": False, "local_eval_read": False, "parent_replay_oof_max_abs": parent_oof_replay_max_abs, "parent_replay_test_max_abs": parent_test_replay_max_abs})
