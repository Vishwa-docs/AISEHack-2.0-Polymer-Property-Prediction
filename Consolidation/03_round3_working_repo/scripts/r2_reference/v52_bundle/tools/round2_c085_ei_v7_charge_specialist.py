#!/usr/bin/env python3
"""Official-only exact-v7 Ei charge specialist screen."""
...
import round2_mixed_candidate_v7 as frozen_v7
...
def masked_target_dense(
    ...
    allowed = np.isin(key_groups, list(allowed_groups))
    values[~allowed] = np.nan
    available[~allowed] = 0.0
    return reference.target_dense_features(base_dense, values, available, target)
...
def masked_maps(
    ...
    return {
        target: {key: value for key, value in values.items() if canonical_groups.get(key) in allowed_groups}
        for target, values in maps.items()
    }
...
def strict_specialized_split(
    ...
    for fold in range(4):
        ...
        local_dense = masked_target_dense(base_dense, cross_values, cross_available, TARGET, key_groups, set(groups[local_train]))
        inner_arms[local_positions] = reference.predict_base_models(
            local_dense, sparse_parts, fingerprints, y_global,
    ...
    outer_dense = masked_target_dense(base_dense, cross_values, cross_available, TARGET, key_groups, set(groups[outer_train]))
    outer_arms = reference.predict_base_models(
        outer_dense, sparse_parts, fingerprints, y_global,
    ...
    for fold in range(4):
        ...
        local_maps = masked_maps(maps, canonical_groups, set(groups[local_train]))
        raw, _, _, _ = ei_route.route_predictions(
            TARGET, canonical[local_validation], inner_base[local_positions], y[local_train], gap_model, local_maps,
        )
    ...
    outer_maps = masked_maps(maps, canonical_groups, set(groups[outer_train]))
    raw, _, _, _ = ei_route.route_predictions(
        TARGET, canonical[outer_validation], outer_base, y[outer_train], gap_model, outer_maps,
    )
...
def strict_specialized_test(
    ...
    for fold in range(4):
        ...
        local_dense = masked_target_dense(base_dense, cross_values, cross_available, TARGET, key_groups, set(groups[training]))
    ...
    full_dense = masked_target_dense(base_dense, cross_values, cross_available, TARGET, key_groups, set(groups))
    test_arms = reference.predict_base_models(
        full_dense, sparse_parts, fingerprints, y_global,
    ...
    full_maps = masked_maps(maps, canonical_groups, set(groups))
    raw, _, _, _ = ei_route.route_predictions(TARGET, test_canonical, parent, y, gap_model, full_maps)
...
def main() -> None:
    ...
    electronic_train_finite_rows = int(np.sum(np.isfinite(electronic[rows]).all(axis=1)))
    electronic_test_finite_rows = int(np.sum(np.isfinite(electronic[test_rows]).all(axis=1)))
    electronic_finite_rows = electronic_train_finite_rows + electronic_test_finite_rows
    ...
    key_groups = np.asarray([plumbing.no_stereo(value) for value in keys], dtype=object)
    canonical_groups = {str(value): str(group) for value, group in zip(keys, key_groups, strict=True)}
    ...
    test_support_keys = {
        value for value in test_frame["canonical"].astype(str).to_numpy()
        if np.isfinite(maps["eea"].get(value, np.nan)) and np.isfinite(maps["egc"].get(value, np.nan))
    }
    oof_support_keys = {
        value for value in frame["canonical"].astype(str).to_numpy()
        if np.isfinite(maps["eea"].get(value, np.nan)) and np.isfinite(maps["egc"].get(value, np.nan))
    }

    special_oof, special_test, special_report = frozen_v7.specialized_target(
        TARGET, pooled, test, keys, dense_base, cross_values, cross_available,
        sparse_parts, fingerprints, config,
    )
    special_oof_replay, special_test_replay, _ = frozen_v7.specialized_target(
        TARGET, pooled, test, keys, dense_base, cross_values, cross_available,
        sparse_parts, fingerprints, config,
    )
    parent_rows = special_oof.reset_index(drop=True)
    parent_rows_replay = special_oof_replay.reset_index(drop=True)
    ...
    parent_oof = parent_rows["candidate"].to_numpy(float)
    replay_oof = parent_rows_replay["candidate"].to_numpy(float)
    parent_test = special_test["target"].to_numpy(float)
    replay_test = special_test_replay["target"].to_numpy(float)
    ...
    for fold in range(5):
        ...
        strict_parent = parent_oof[validation]
        inner_parent = parent_oof[training]
        fitted = model()
        fitted.fit(features[rows[training]], y[training] - inner_parent)
        candidate[validation] = strict_parent
        supported_validation = np.isin(frame.iloc[validation]["canonical"].astype(str), list(oof_support_keys))
        correction = RESIDUAL_WEIGHT * fitted.predict(features[rows[validation]])
        candidate[validation[supported_validation]] += correction[supported_validation]
    ...
    supported_test = np.isin(test_frame["canonical"].astype(str), list(test_support_keys))
    ...
    supported_oof = np.isin(frame["canonical"].astype(str), list(oof_support_keys))
    missing_oof = ~supported_oof
    ...
    support_delta = panel_delta(supported_oof)
    missing_delta = panel_delta(missing_oof)
    panels["deployment_supported_eea_egc"] = {"rows": int(np.sum(supported_oof)), "delta_r2": support_delta, "status": "evaluable" if support_delta is not None else "insufficient"}
    panels["deployment_unsupported_parent_only"] = {"rows": int(np.sum(missing_oof)), "delta_r2": missing_delta, "status": "unchanged_control"}
    ...
    gates = {"gain_pass": delta >= 0.01, "fold_pass": sum(row["delta_r2"] > 0 for row in fold_rows) >= 4, "bootstrap_pass": lower > 0.0, "panel_pass": minimum_panel is not None and minimum_panel >= 0.0, "component_rows_pass": len(component) == 148, "parent_replay_oof_pass": parent_oof_replay_max_abs <= 1.0e-12, "parent_replay_test_pass": parent_test_replay_max_abs <= 1.0e-12, "electronic_feature_support_pass": electronic_train_finite_rows == len(y) and electronic_test_finite_rows == len(test_frame), "missing_support_control_pass": missing_delta is None or abs(missing_delta) <= 1.0e-12}
    passed = bool(all(gates.values()))
    source_names = ("round2_c085_ei_v7_charge_specialist.py", "round2_mixed_candidate_v7.py", "round2_c075_ei_cross_target_masked_charge_ridge.py", "round2_c063_egb_endpoint_conjugation_residual.py", "round2_ei_scaffold_abstaining_gap_identity_v4_portable.py", "initial_reference_pipeline.py", "round2_eea_cross_target_oof_residual_stack.py")
    report = {
        "schema_version": "ppp.round2.c085.ei-v7-charge-specialist.v1",
        "experiment_id": run_dir.name,
        "created_at": datetime.now().astimezone().isoformat(),
        "parent": "R2-C050-20260803-2130-mixed-c001-gap-components-v7; regenerated frozen-v7 Ei route with row-local Eea/Egc support-masked Ridge residual",
        ...
        "electronic_feature_train_finite_rows": electronic_train_finite_rows,
        "electronic_feature_test_finite_rows": electronic_test_finite_rows,
    ...
    write_json(run_dir / "config.json", {"seed": SEED, "target": TARGET, "feature_family": "official-SMILES endpoint/conjugation/dielectric density plus dummy-capped Gasteiger charge and charge-distance descriptors", "model": "Ridge", "ridge_alpha": RIDGE_ALPHA, "residual_weight": RESIDUAL_WEIGHT, "outer": "canonical_no_stereo GroupKFold(5) with outer-training-only inner parent/gap labels and fold-masked cross-target covariates/maps", "bootstrap_resamples": 2000, "no_hyperparameter_sweep": True, "external_label_file_read": False, "local_eval_read": False, "parent_replay_oof_max_abs": parent_oof_replay_max_abs, "parent_replay_test_max_abs": parent_test_replay_max_abs, "electronic_feature_finite_rows": electronic_finite_rows, "electronic_feature_train_finite_rows": electronic_train_finite_rows, "electronic_feature_test_finite_rows": electronic_test_finite_rows})
