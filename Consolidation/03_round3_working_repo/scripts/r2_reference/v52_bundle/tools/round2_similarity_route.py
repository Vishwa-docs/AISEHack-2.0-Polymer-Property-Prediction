#!/usr/bin/env python3
"""Fixed clean similarity-gated route over previously measured C013 arms."""
...
# (alpha, similarity intervals applied to the new arm; all other bins retain C001)
ROUTES = {
...
def evaluate_route(y: np.ndarray, baseline: np.ndarray, arm: np.ndarray, groups: np.ndarray, folds: np.ndarray, nearest: np.ndarray, spec: dict[str, Any] | None) -> dict[str, Any]:
    changed = np.zeros(len(y), dtype=bool)
    if spec is not None:
        for lower, upper in spec["bins"]:
            changed |= (nearest >= lower) & (nearest < upper)
    alpha = float(spec["alpha"]) if spec is not None else 0.0
    prediction = baseline.copy()
    prediction[changed] = baseline[changed] + alpha * (arm[changed] - baseline[changed])
    fold_rows = []
    for fold in range(5):
        selected = folds == fold
        fold_rows.append({"fold": fold, "rows": int(np.sum(selected)), "baseline_r2": r2(y[selected], baseline[selected]), "route_r2": r2(y[selected], prediction[selected]), "delta_r2": r2(y[selected], prediction[selected]) - r2(y[selected], baseline[selected])})
    low_rows = {}
    for name, lower, upper in (("lt_0.30", 0.0, 0.30), ("0.30_0.50", 0.30, 0.50), ("0.50_0.70", 0.50, 0.70), ("ge_0.70", 0.70, 1.01)):
        ...
        if int(np.sum(selected)) < 5:
            continue
        low_rows[name] = {"rows": int(np.sum(selected)), "changed_rows": int(np.sum(changed & selected)), "baseline_r2": r2(y[selected], baseline[selected]), "route_r2": r2(y[selected], prediction[selected]), "delta_r2": r2(y[selected], prediction[selected]) - r2(y[selected], baseline[selected])}
    delta = prediction - baseline
    changed_delta = delta[changed]
    changed_groups = groups[changed]
    min_low = min((row["delta_r2"] for row in low_rows.values()), default=0.0)
    return {
        ...
        "bootstrap_lower_changed": group_bootstrap_lower(changed_delta, changed_groups),
        "low_similarity": low_rows,
        "min_low_similarity_delta": min_low,
        "folds": fold_rows,
    }
...
def main() -> None:
    ...
    for target_index, target in enumerate(TARGETS):
        ...
        model_name = route_spec["model"] if route_spec is not None else MODEL_FOR_TARGET[target]
        for fold in range(5):
            ...
            model = zoo.model_factory(model_name, 2026 + target_index)
        ...
        baseline_score = evaluate_route(y, baseline, arm_prediction, groups, folds, nearest, None)
        route_score = evaluate_route(y, baseline, arm_prediction, groups, folds, nearest, route_spec)
        route_score["baseline_r2"] = baseline_score["r2"]
        route_score["model_name"] = model_name
        route_score["selected_route"] = route_spec is not None
        route_score["pass_component"] = bool(route_spec is None or (route_score["delta_r2"] > 0.0 and route_score["bootstrap_lower_changed"] > 0.0 and route_score["min_low_similarity_delta"] >= 0.0))
        reports[target] = {"baseline": baseline_score, "route": route_score}
        route_rows.append({"target": target, "model": model_name, "alpha": route_score["alpha"], "changed_rows": route_score["changed_rows"], "baseline_r2": route_score["baseline_r2"], "route_r2": route_score["r2"], "delta_r2": route_score["delta_r2"], "bootstrap_lower_changed": route_score["bootstrap_lower_changed"], "min_low_similarity_delta": route_score["min_low_similarity_delta"], "pass_component": route_score["pass_component"]})
    ...
    audit = {"schema_version": "ppp.round2.similarity-route-run.v1", "experiment_id": run_dir.name, "created_at": datetime.now().astimezone().isoformat(), "parent": "R2-C016-20260803-1830-target-blend", "official_inputs": inputs, "baseline_grouped_mean_r2": baseline_mean, "route_grouped_mean_r2": route_mean, "route_mean_gain": route_mean - baseline_mean, "worst_target_delta": worst_delta, "routes": ROUTES, "targets": reports, "route": route_rows, "full_pass": full_pass, "decision": "prospective_route_pass" if full_pass else "rejected_similarity_route_gate", "elapsed_seconds": float(time.time() - started)}
    pd.DataFrame(route_rows).to_csv(run_dir / "metrics.csv", index=False)
    write_json(run_dir / "metrics.json", audit)
    write_json(run_dir / "config.json", {"schema_version": "ppp.round2.similarity-route.v1", "seed": 2026, "folds": 5, "routes": ROUTES, "model_for_target": MODEL_FOR_TARGET, "official_inputs": inputs})
