#!/usr/bin/env python3
"""C213: deterministic clean component compound audit v8.

This audit-only child runs after C212. It preserves C211's C208 Tg, C207 Egc,
and C210 Nc priorities, adds C212 as the first Nc priority, and preserves the
existing banked Ei/Eea/EPS priorities and strict eligibility checks.
...
actions. Missing or failed C207/C208/C210/C212 metrics are skipped with
explicit reasons.
"""
...
def main() -> None:
    ...
    if len(test_prediction) != 4940 or not np.array_equal(test_prediction["id"].to_numpy(int), np.arange(1, 4941)):
        raise RuntimeError("C213 ID coverage/order contract failed")
    if not np.isfinite(test_prediction["target"].to_numpy(float)).all():
        raise RuntimeError("C213 produced non-finite predictions")
    ...
    report = {
        "schema_version": "ppp.round2.c213.clean-component-compound-audit.v8",
