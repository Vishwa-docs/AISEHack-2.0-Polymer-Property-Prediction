"""C146: target-excluded chi-coordinate routing for Ei/Eea one-partner rows."""
...
chi_rows = np.where(obs[:, ti["ei"]] & obs[:, ti["eea"]])[0]
chi_y = (labels[chi_rows, ti["ei"]] + labels[chi_rows, ti["eea"]]) / 2
base = np.load(SCR / "out_clean_corrected/PFINAL.npy")
base_ei = base[ei_rows, ti["ei"]].copy()
ei_pos = {r: i for i, r in enumerate(ei_rows)}
# Fixed C143-style identity for the both-partners slice, then test the new
# one-partner chi route only on Eea-observed/Ei-target rows.
...
for fold, (tr, va) in enumerate(KFold(5, shuffle=True, random_state=SEED + 146).split(chi_rows), 1):
    train_rows = chi_rows[tr]; val_rows = chi_rows[va]
    fit = fit_ridge(X[train_rows], chi_y[tr], alpha=50.0)
    # Only Eea-observed, Ei-validation rows use the new formula.  Both-partner
    # rows remain the fixed observed-label identity route above.
    routed = [r for r in val_rows if obs[r, ti["eea"]] and not obs[r, ti["egc"]]]
    if routed:
        chi_hat = pred(fit, X[np.asarray(routed, dtype=int)])
        for r, ch in zip(routed, chi_hat):
            raw = 2 * ch - labels[r, ti["eea"]]
            i = ei_pos[r]
            candidate_ei[i] = .5 * base_ei[i] + .5 * raw
    folds.append({"fold": fold, "fit_rows": int(len(train_rows)), "validation_rows": int(len(val_rows)), "routed_e ea_only": int(len(routed))})
...
print("C146 Ei OOF", json.dumps(metrics, sort_keys=True), flush=True)

full_fit = fit_ridge(X[chi_rows], chi_y, alpha=50.0)
...
chi_test = pred(full_fit, X[test_fi])
for i, row in test.iterrows():
    fi = int(row.fi)
    if row.target_type == "ei" and obs[fi, ti["eea"]] and not obs[fi, ti["egc"]]:
        raw = 2 * chi_test[i] - labels[fi, ti["eea"]]
        candidate.loc[i, "target"] = .5 * candidate.loc[i, "target"] + .5 * raw

name = "R2-C146-ei-chi-one-partner-LOCAL_DIAGNOSTIC_ONLY"
path = OUT / f"{name}.csv"; candidate.to_csv(path, index=False)
report = {"experiment": name, "official_only_fitting": True, "local_eval_read": False, "pretrained_weights": False, "mechanism": "structure-only Ridge chi=(Ei+Eea)/2, 5-fold target-excluded fit, fixed 0.5 blend, Eea-observed/Egc-missing rows only", "chi_rows": int(len(chi_rows)), "folds": folds, "metrics": metrics, "candidate_path": str(path), "elapsed_seconds": time.time() - started}
(OUT / f"{name}-oof.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
print("C146 candidate", path, len(candidate), flush=True)
