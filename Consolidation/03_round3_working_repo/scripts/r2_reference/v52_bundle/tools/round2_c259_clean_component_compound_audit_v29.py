#!/usr/bin/env python3
"""C259: deterministic compound audit v29 with C258 Ei priority."""
...
import round2_c257_clean_component_compound_audit_v28 as c257


SCHEMA = "ppp.round2.c259.clean-component-compound-audit.v29"
C258_ID = "R2-C258-20260805-1010-ei-eht-orbital-residual-v1"
CHANGED_FACTOR = (
    "Insert C258 RDKit/YAeHMOP EHT Ei residual as first Ei priority, consuming it only if "
    "it independently banks Ei under the strict selected-C199-reference gate; preserve C257 priorities otherwise."
)
...
def post_patch(run_dir: Path) -> None:
    ...
    metrics["selection_rule"] = (
        "first completed clean-passing target component in frozen priority order; C258 first for Ei "
        "only if its own metrics bank Ei; no local_eval/public feedback; no same-OOF max search"
    )
    source_hashes = dict(metrics.get("source_hashes", {}))
    source_hashes["c257_wrapper_runner"] = h.sha256_file(Path(c257.__file__).resolve())
...
def main() -> None:
    ...
    base.c203.metric_passes = c257.c255.c253.c249.c245.c243.c241.c239.c237.c235.c233.c231.c229.c227.guarded_metric_passes
    base.main()
