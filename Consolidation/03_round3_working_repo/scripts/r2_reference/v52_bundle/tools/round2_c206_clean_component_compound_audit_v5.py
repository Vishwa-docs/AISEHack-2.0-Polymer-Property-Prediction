#!/usr/bin/env python3
"""C206: deterministic clean component compound audit v5.

This is audit-only glue.  It adds C204 as first Eea priority and C205 as first
Ei priority, then falls back to the previously banked C199/C189/C190 components
only when the newer child is missing or fails its own component gate.

The priority order is frozen before C204/C205 terminal results are known.  This
script does not introduce a model family, does not select same-OOF maxima, does
not read local_eval/public feedback, and does not authorize notebook/upload/submit
actions.
"""
...
def main() -> None:
    ...
    if len(test_prediction) != 4940 or not np.array_equal(test_prediction["id"].to_numpy(int), np.arange(1, 4941)):
        raise RuntimeError("C206 ID coverage/order contract failed")
    if not np.isfinite(test_prediction["target"].to_numpy(float)).all():
        raise RuntimeError("C206 produced non-finite predictions")
    ...
    report = {
        "schema_version": "ppp.round2.c206.clean-component-compound-audit.v5",
