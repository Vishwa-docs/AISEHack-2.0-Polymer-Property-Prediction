#!/usr/bin/env python3
"""Official-only fully nested EPS specialist screen."""
...
def main() -> None:
    ...
    source_names = ("round2_c084_eps_fully_nested_specialist.py", "round2_c076_eps_paired_charge_polarizability_residual.py", "round2_c063_egb_endpoint_conjugation_residual.py", "initial_reference_pipeline.py", "round2_eea_cross_target_oof_residual_stack.py")
    report = {
        "schema_version": "ppp.round2.c084.eps-fully-nested-specialist.v1",
        "experiment_id": run_dir.name,
        "created_at": datetime.now().astimezone().isoformat(),
        "parent": "R2-C050-20260803-2130-mixed-c001-gap-components-v7; compact fully nested EPS parent/blend regenerated twice",
    ...
    (run_dir / "decision.md").write_text(f"# {run_dir.name}\n\nDecision: **{report['decision']}**. EPS fully nested specialist parent R2 `{parent_r2:.12f}`, candidate R2 `{candidate_r2:.12f}`, delta `{delta:+.12f}`. Official-only; no local_eval, external_label file, or Kaggle action.\n", encoding="utf-8")
    manifest = []
