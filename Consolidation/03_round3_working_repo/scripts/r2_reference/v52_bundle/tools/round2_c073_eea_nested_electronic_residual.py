#!/usr/bin/env python3
"""Official-only properly nested Eea electronic residual screen."""
...
def charge_features(molecules: list[Chem.Mol], indices: list[int]) -> tuple[np.ndarray, list[str]]:
    names = ["charge_mean", "charge_std", "charge_min", "charge_max", "charge_range", "abs_charge_mean", "abs_charge_sum_density", "hetero_charge_abs_mean", "charge_weighted_distance", "charge_weighted_hetero_distance"]
    output = np.full((len(indices), len(names)), np.nan, dtype=np.float64)
    for row, source_index in enumerate(indices):
        molecule = molecules[source_index]
        try:
            ...
            if not finite.any():
                continue
            charges = np.where(finite, charges, 0.0)
            atoms = list(molecule.GetAtoms())
            hetero = np.asarray([atom.GetAtomicNum() not in (0, 1, 6) for atom in atoms], dtype=bool)
            pair_weight = np.abs(charges[:, None] * charges[None, :])
            norm = max(float(pair_weight.sum()), 1.0e-12)
            hetero_pair = pair_weight * (hetero[:, None] | hetero[None, :])
            hetero_norm = max(float(hetero_pair.sum()), 1.0e-12)
            heavy = max(molecule.GetNumHeavyAtoms(), 1)
            output[row] = [
                float(np.mean(charges)), float(np.std(charges)), float(np.min(charges)), float(np.max(charges)), float(np.ptp(charges)),
                float(np.mean(np.abs(charges))), float(np.sum(np.abs(charges)) / heavy),
                float(np.mean(np.abs(charges[hetero]))) if np.any(hetero) else 0.0,
                float((pair_weight * distances).sum() / norm),
                float((hetero_pair * distances).sum() / hetero_norm),
            ]
        except Exception:
            continue
    output[~np.isfinite(output)] = np.nan
...
def main() -> None:
    ...
    parent_oof_frame, parent_test_frame, _ = v7.specialized_target(
        TARGET, pooled, test, keys, dense_base, cross_values, cross_available,
        sparse_parts, fingerprints, config,
    )
    parent_test_frame = parent_test_frame.sort_values("id").reset_index(drop=True)
    replay_oof_frame, replay_test_frame, _ = v7.specialized_target(
        TARGET, pooled, test, keys, dense_base, cross_values, cross_available,
        sparse_parts, fingerprints, config,
    )
    replay_test_frame = replay_test_frame.sort_values("id").reset_index(drop=True)
    frame = pooled[pooled["target_type"] == TARGET].reset_index(drop=True)
    parent_rows = parent_oof_frame.reset_index(drop=True)
    replay_rows = replay_oof_frame.reset_index(drop=True)
    ...
    if not np.array_equal(frame["canonical"].astype(str).to_numpy(), replay_rows["canonical"].astype(str).to_numpy()):
        raise RuntimeError("exact v7 Eea replay row alignment failed")
    y = frame["target"].to_numpy(float)
    parent_oof = parent_rows["candidate"].to_numpy(float)
    replay_oof = replay_rows["candidate"].to_numpy(float)
    ...
    features = np.hstack([base_features, extra_features, electronic_features])
    ...
    if not np.isfinite(candidate).all():
        raise RuntimeError("non-finite Eea OOF candidate")
    ...
    test_parent = parent_test_frame["target"].to_numpy(float)
    test_replay_parent = replay_test_frame["target"].to_numpy(float)
    ...
    gates = {"gain_pass": delta >= 0.01, "fold_pass": sum(row["delta_r2"] > 0 for row in fold_rows) >= 4, "bootstrap_pass": lower > 0.0, "panel_pass": minimum_panel >= 0.0, "component_rows_pass": len(component) == 147, "parent_replay_oof_pass": parent_oof_replay_max_abs <= 1.0e-12, "parent_replay_test_pass": parent_test_replay_max_abs <= 1.0e-12}
    passed = bool(all(gates.values()))
    source_names = ("round2_c073_eea_nested_electronic_residual.py", "round2_c068_eps_physics_informed_residual.py", "round2_c063_egb_endpoint_conjugation_residual.py", "round2_mixed_candidate_v7.py", "initial_reference_pipeline.py", "round2_eea_cross_target_oof_residual_stack.py")
    report = {
        "schema_version": "ppp.round2.c073.eea-nested-electronic-residual.v1",
        "experiment_id": run_dir.name,
        "created_at": datetime.now().astimezone().isoformat(),
        "parent": "R2-C050-20260803-2130-mixed-c001-gap-components-v7; exact v7 Eea specialized parent regenerated twice in-process",
    ...
    write_json(run_dir / "config.json", {"seed": SEED, "target": TARGET, "feature_family": "official-SMILES endpoint/conjugation/dielectric density plus Gasteiger charge and charge-distance descriptors", "model": "Ridge", "ridge_alpha": ALPHA, "residual_weight": RESIDUAL_WEIGHT, "outer": "canonical_no_stereo GroupKFold(5)", "bootstrap_resamples": 2000, "no_hyperparameter_sweep": True, "external_label_file_read": False, "local_eval_read": False, "parent_replay_oof_max_abs": parent_oof_replay_max_abs, "parent_replay_test_max_abs": parent_test_replay_max_abs})
