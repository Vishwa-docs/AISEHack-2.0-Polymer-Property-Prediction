...
def nested_split(
    ...
    for fold in range(4):
        local_train = np.flatnonzero(inner_folds != fold)
        local_validation = np.flatnonzero(inner_folds == fold)
        inner_parent[inner_folds == fold] = c050_parent(
            y[outer_train],
            groups[outer_train],
            local_train,
            local_validation,
...
def main() -> None:
    ...
    for fold in range(5):
        ...
        result = nested_split(
            ...
            key_groups, sparse_parts, fingerprints, y_global, indices, isomeric, scaffolds, aux_info
        )
        replay_parent = c050_parent(
            y,
            groups,
            training,
            validation,
            ...
            isomeric,
            scaffolds,
            f"c095_replay_outer_{validation[0]}",
        )
    ...
    test_parent = c050_parent_test(y, groups, target_dense, sparse_parts, fingerprints, indices, test_global_indices, y_global)
    ...
    script_path = root / "tools" / "round2_c095_ei_c050_parent_cross_target_residual_v1.py"
    ...
    audit = {
        "schema_version": "ppp.round2.c095.ei-c050-parent-cross-target-residual-run.v1",
        "experiment_id": run_dir.name,
        "created_at": datetime.now().astimezone().isoformat(),
        "parent": "C050-v7 Ei scaffold/gap parent regenerated from source",
    ...
    write_json(run_dir / "config.json", {
        "schema_version": "ppp.round2.c095.ei-c050-parent-cross-target-residual.v1",
        "seed": 2026,
        "target": TARGET,
        "auxiliary_targets": AUXILIARY,
        "outer_folds": 5,
    ...
    manifest.extend([
        f"{audit['source_hashes']['script']}  SOURCE tools/round2_c095_ei_c050_parent_cross_target_residual_v1.py",
