#!/usr/bin/env python3
"""Strictly nested official-only Ei/Eea gap-identity component experiment."""
...
def target_panels(
    ...
    routed: np.ndarray,
    partner_or_egc_missing: np.ndarray,
    groups: np.ndarray,
    ...
    add("partner_and_egc_available", routed)
    add("partner_or_egc_missing_parent_only", partner_or_egc_missing, unchanged_control=True)
...
def main() -> None:
    ...
    config.update({"seed": 2026, "folds": 5, "gap_alpha": GAP_ALPHA, "parent_cross_property_covariates": "clean_baseline_except_own_target"})
    ...
    for target in TARGETS:
        ...
        for fold in range(5):
            ...
            gap_rows.append({"target": target, "fold": fold, "gap_training_groups": gap_count, "routed_rows": int(np.sum(route))})
        ...
        panels, min_panel, panel_incomplete = target_panels(
            y, baseline, candidate, nearest, scaffolds, measurements, routed, missing, groups,
        )
        ...
        target_pass = bool(
            ...
            and float(target_reports["route_definition"]["missing_rows_max_change"]) <= 1.0e-12
        )
        ...
        test_route = np.isfinite(test_partner) & np.isfinite(test_egc) & (full_gap_model is not None)
        test_pred_gap = np.full(len(test_frame), np.nan, dtype=float)
        if full_gap_model is not None and np.any(test_route):
            test_pred_gap[test_route] = full_gap_model.predict(test_egc[test_route, None])
        test_reconstruction = np.full(len(test_frame), np.nan, dtype=float)
        if np.any(test_route):
            test_reconstruction[test_route] = test_partner[test_route] + test_pred_gap[test_route] if target == "ei" else test_partner[test_route] - test_pred_gap[test_route]
        test_route_rows.extend(pd.DataFrame({
            ...
            "gap_training_groups": full_gap_count,
        }).to_dict("records"))
    ...
    metrics = {
        "schema_version": "ppp.round2.ei-eea-gap-identity-run.v1",
        "experiment_id": run_dir.name,
        "created_at": datetime.now().astimezone().isoformat(),
        "parent": "R2-C040-20260803-2300-egb-periodic-electronic-absolute",
    ...
    manifest.extend([
        f"{source_hashes['script']}  SOURCE tools/round2_ei_eea_gap_identity.py",
