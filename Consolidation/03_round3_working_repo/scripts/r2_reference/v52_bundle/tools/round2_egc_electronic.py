#!/usr/bin/env python3
"""Bounded official-only Egc electronic/conjugation carrier diagnostic."""
...
ARMS = ("electronic_histgb", "electronic_lightgbm", "electronic_ridge")
...
def preprocess(matrix: np.ndarray, train_indices: np.ndarray, validation_indices: np.ndarray) -> tuple[np.ndarray, np.ndarray, int, StandardScaler]:
    imputer = SimpleImputer(strategy="median", keep_empty_features=True)
    train_x = np.asarray(matrix[train_indices], dtype=np.float64).copy()
    validation_x = np.asarray(matrix[validation_indices], dtype=np.float64).copy()
    ...
    if not np.any(keep):
        raise RuntimeError("no nonconstant electronic features remained")
    ...
    return train_x, validation_x, int(np.sum(keep)), (scaler, scaled_train, scaled_validation)
...
def main() -> None:
    ...
    target_dense = reference.target_dense_features(rich_dense, cross_values, cross_available, TARGET)
    ...
    for fold in range(5):
        ...
        base = reference.predict_base_models(baseline_dense, sparse_parts, fingerprints, y_global, indices[train_rows], indices[validation_rows], reference.DEFAULT_CONFIG, TARGET)
        ...
        train_x, validation_x, kept, scaled = preprocess(target_dense, indices[train_rows], indices[validation_rows])
        retained_features.append(kept)
        _, scaled_train, scaled_validation = scaled
        models = [
            HistGradientBoostingRegressor(max_iter=350, learning_rate=0.04, max_leaf_nodes=31, min_samples_leaf=12, l2_regularization=0.10, random_state=2026 + fold),
            lgb.LGBMRegressor(objective="regression", n_estimators=350, learning_rate=0.03, num_leaves=31, min_child_samples=12, subsample=0.9, subsample_freq=1, colsample_bytree=0.8, reg_lambda=4.0, random_state=2030 + fold, n_jobs=1, verbosity=-1),
            Ridge(alpha=30.0),
        ]
        matrices = [(train_x, validation_x), (train_x, validation_x), (scaled_train, scaled_validation)]
    ...
    for name in ARMS:
        ...
        arm = {"r2": r2(y, prediction), "delta_r2": r2(y, prediction) - report["baseline_r2"], "folds": fold_reports[name]}
        ...
        arm["pass"] = bool(arm["delta_r2"] >= 0.01 and arm["positive_folds"] >= 4 and arm["bootstrap_lower"] > 0.0 and (not low_values or min(low_values) >= 0.0))
    ...
    audit = {"schema_version": "ppp.round2.egc-electronic-run.v1", "experiment_id": run_dir.name, "created_at": datetime.now().astimezone().isoformat(), "parent": "R2-C013-20260803-1804-target-tree-zoo-v2", "official_inputs": inputs, "target": TARGET, "report": report, "passing_arms": passing, "decision": "component_pass" if passing else "rejected_component_gate", "elapsed_seconds": float(time.time() - started)}
    rows = [{"target": TARGET, "arm": name, "r2": report["arms"][name]["r2"], "delta_r2": report["arms"][name]["delta_r2"], "positive_folds": report["arms"][name]["positive_folds"], "bootstrap_lower": report["arms"][name]["bootstrap_lower"], "min_low_similarity_delta": report["arms"][name]["min_low_similarity_delta"], "pass": report["arms"][name]["pass"]} for name in ARMS]
    pd.DataFrame(rows).to_csv(run_dir / "metrics.csv", index=False)
    write_json(run_dir / "metrics.json", audit)
    write_json(run_dir / "config.json", {"schema_version": "ppp.round2.egc-electronic.v1", "seed": 2026, "folds": 5, "target": TARGET, "arms": list(ARMS), "feature_count": int(rich_dense.shape[1]), "official_inputs": inputs})
    ...
    print(json.dumps({"experiment_id": run_dir.name, "decision": audit["decision"], "passing_arms": passing, "elapsed_seconds": audit["elapsed_seconds"]}, indent=2), flush=True)
