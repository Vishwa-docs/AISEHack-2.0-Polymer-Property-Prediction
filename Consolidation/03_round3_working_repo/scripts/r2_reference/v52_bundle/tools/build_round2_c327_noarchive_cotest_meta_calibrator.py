#!/usr/bin/env python3
"""C327 no-archive weak-target co-test residual meta-calibrator.

This is a local diagnostic builder for the without-archive branch only.  It
starts from the frozen C312 no-archive compound and tests a small residual
calibrator for ei/eea/eps/nc.  The calibrator is trained from official current
train labels and C282 current-only OOF predictions; it reads no local_eval/external_label
files and uses no archive or PI1M inputs.

If a target fails the clean OOF gate, that target is left byte/numerically
unchanged from the C312 base.  LocalEval/proxy scoring is deliberately left to the
separate post-freeze scorer after this script has written and hashed the CSV.
"""
...
CONFIGS = {
    ...
    "eea": TargetConfig(0.003, 4, -0.003, 0.45, 0.75),
...
def guard_path(path: Path, *, allow_output: bool = False) -> None:
    ...
    if not allow_output:
        if "/archive/" in low or low.endswith("/archive") or "with_archive" in low:
            raise RuntimeError(f"Refusing archive/cross-branch input path for C327: {path}")
...
def parse_targets(value: str) -> tuple[str, ...]:
    ...
    if invalid:
        raise RuntimeError(f"C327 only supports {ACTIVE_TARGETS}, got {invalid}")
    return targets
...
def load_base(path: Path, ids: np.ndarray) -> pd.DataFrame:
    ...
    if "without_archive" not in str(path):
        raise RuntimeError(f"C327 base must be branch-local without_archive: {path}")
...
def load_inputs(data_dir: Path, base_path: Path, oof_path: Path) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    for target in ACTIVE_TARGETS:
        ...
        # C282 has full coverage for the original weak targets, but a few Tg
        # rows can be absent after canonicalization/fit filtering.  The
        # calibrator evaluates only rows present in the frozen OOF file, so
        # fail only on material coverage loss.
...
def main() -> None:
    ...
    if "without_archive" not in str(output):
        raise RuntimeError(f"C327 output must live in without_archive namespace: {output}")
    ...
    record = {
        "schema_version": "ppp.round2.c327.noarchive-cotest-meta-calibrator.v1",
        ...
        "method": "weak-target co-test residual meta-calibrator over C312 with C282 OOF clean gates",
