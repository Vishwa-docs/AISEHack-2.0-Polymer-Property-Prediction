#!/usr/bin/env python3
"""Official-only EPS nonlinear multiview residual screen."""
...
def fingerprint_features(molecules: list[Chem.Mol], indices: list[int], bits: int = 512) -> tuple[np.ndarray, list[str]]:
        ...
        try:
            ...
            for atom in capped.GetAtoms():
                if atom.GetAtomicNum() == 0:
                    atom.SetAtomicNum(6); atom.SetFormalCharge(0); atom.SetNoImplicit(False)
            molecule = capped.GetMol(); Chem.SanitizeMol(molecule); rdPartialCharges.ComputeGasteigerCharges(molecule)
            ...
            atoms = list(molecule.GetAtoms()); hetero = np.asarray([atom.GetAtomicNum() not in (0, 1, 6) for atom in atoms], dtype=bool)
            weights = np.abs(charges[:, None] * charges[None, :]); norm = max(float(weights.sum()), 1.0e-12)
            hetero_weights = weights * (hetero[:, None] | hetero[None, :]); hetero_norm = max(float(hetero_weights.sum()), 1.0e-12)
            heavy = max(molecule.GetNumHeavyAtoms(), 1)
...
def main() -> None:
    ...
    if not np.array_equal(frame["canonical"].astype(str).to_numpy(), replay_parent_rows["canonical"].astype(str).to_numpy()):
        raise RuntimeError("exact v7 EPS replay row alignment failed")
    ...
    charge, charge_names = charge_features(molecules, target_indices)
    morgan2 = np.asarray(reference.morgan_count_matrix([molecules[index] for index in target_indices], 2, 2048).toarray(), dtype=np.float64)
    morgan3 = np.asarray(reference.morgan_count_matrix([molecules[index] for index in target_indices], 3, 2048).toarray(), dtype=np.float64)
    morgan_names = [f"morgan2_count_{index}" for index in range(morgan2.shape[1])] + [f"morgan3_count_{index}" for index in range(morgan3.shape[1])]
    features = np.hstack([base_features, extra_features, charge, pair_features, morgan2, morgan3])
    feature_names = base_names + extra_names + charge_names + pair_names + morgan_names
    ...
    replay_test_parent = replay_detail[replay_detail["target_type"] == TARGET].sort_values("id")["model_prediction"].to_numpy(float)
    parent_test_replay_max_abs = float(np.max(np.abs(test_parent - replay_test_parent)))
    ...
    source_names = ("round2_c080_eps_nonlinear_multiview_residual.py", "round2_c069_eps_physics_extra_trees_residual.py", "round2_c071_nc_atom_pair_torsion_residual.py", "round2_c063_egb_endpoint_conjugation_residual.py", "initial_reference_pipeline.py", "round2_eea_cross_target_oof_residual_stack.py")
    report = {
        "schema_version": "ppp.round2.c080.eps-nonlinear-multiview-residual.v1",
        "experiment_id": run_dir.name,
        "created_at": datetime.now().astimezone().isoformat(),
        "parent": "R2-C050-20260803-2130-mixed-c001-gap-components-v7; exact v7 EPS regenerated twice in-process",
    ...
    write_json(run_dir / "config.json", {"seed": SEED, "target": TARGET, "feature_family": "official-SMILES endpoint/conjugation, dielectric/polarizability, capped-charge, atom-pair/torsion, and Morgan count multiview", "model": "ExtraTreesRegressor", "n_estimators": N_ESTIMATORS, "max_depth": MAX_DEPTH, "min_samples_leaf": MIN_SAMPLES_LEAF, "max_features": MAX_FEATURES, "residual_weight": RESIDUAL_WEIGHT, "outer": "canonical_no_stereo GroupKFold(5)", "bootstrap_resamples": 2000, "no_hyperparameter_sweep": True, "external_label_file_read": False, "local_eval_read": False, "parent_replay_oof_max_abs": parent_oof_replay_max_abs, "parent_replay_test_max_abs": parent_test_replay_max_abs})
    (run_dir / "environment.txt").write_text(f"python={platform.python_version()}\nnumpy={np.__version__}\npandas={pd.__version__}\nrdkit={Chem.rdBase.rdkitVersion}\nplatform={platform.platform()}\n", encoding="utf-8")
    (run_dir / "command.txt").write_text(" ".join(sys.argv) + "\n", encoding="utf-8")
    (run_dir / "decision.md").write_text(f"# {run_dir.name}\n\nDecision: **{report['decision']}**. EPS multiview parent R2 `{parent_r2:.12f}`, candidate R2 `{candidate_r2:.12f}`, delta `{delta:+.12f}`. Official-only; no local_eval, external_label file, or Kaggle action.\n", encoding="utf-8")
    manifest = []
