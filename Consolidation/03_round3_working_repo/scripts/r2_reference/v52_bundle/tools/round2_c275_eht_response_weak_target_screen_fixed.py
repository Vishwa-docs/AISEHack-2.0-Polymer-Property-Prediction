"""C275: fixed-length EHT atom-response screen for EPS, Nc, and Ei."""
...
TOOLS = ROOT / "tools"
RUN = ROOT / "experiments/CLEAN_OFFICIAL_ONLY/R2-C275-20260805-eht-response-weak-target-screen-fixed-v1"
...
def atom_response_features(molecule: Chem.Mol | None, seed: int) -> tuple[list[float], bool]:
    ...
    # Keep the response vector fixed-length at orbital-array boundaries. The
    # original screen used truncated ranges, which produced ragged rows and a
    # runtime-invalid matrix late in the run. Missing edge transitions are
    # represented by deterministic zeros and support is retained separately.
    transition = []
    for occupied_offset in (-2, -1, 0):
        occupied_index = max(0, min(len(energies) - 1, homo_index + occupied_offset))
        for virtual_offset in (0, 1, 2):
            virtual_index = max(0, min(len(energies) - 1, lumo_index + virtual_offset))
            if virtual_index <= occupied_index:
                transition.extend([0.0, 0.0])
                continue
            overlap = np.abs(matrix[:, occupied_index]) * np.abs(matrix[:, virtual_index])
            gap = max(1e-5, abs(float(energies[virtual_index] - energies[occupied_index])))
            transition.extend([float(np.sum(overlap) / gap), float(np.sum(overlap * radius) / gap)])
...
def main() -> None:
    ...
    report = {"schema_version": "ppp.round2.c274.eht-response-fixed.v1", "experiment_id": RUN.name, "status": "feasibility_pass" if screen_pass else "rejected_screen", "official_only": True, "targets": target_reports, "screen_pass": screen_pass, "feature_count": int(matrix.shape[1]), "hcap_supported": int(sum(x["hcap_supported"] for x in support)), "ring_supported": int(sum(x["ring_supported"] for x in support)), "local_eval_read": False, "kaggle_compute": False, "kaggle_upload": False, "kaggle_submission": False, "promotion_eligible": False, "stable_seed": True, "fixed_length_transition_features": True, "elapsed_seconds": time.time() - started, "source_hashes": {"runner": digest(Path(__file__)), "eht": digest(TOOLS / "round2_c258_ei_eht_orbital_residual.py")}}
    ...
    (RUN / "decision.md").write_text(f"# C275 decision\n\nStatus: **{report['status']}**. This is a fixed-length C050 feasibility screen only; any positive target requires selected-parent comparison and full gates before promotion.\n", encoding="utf-8")
    manifest = []
