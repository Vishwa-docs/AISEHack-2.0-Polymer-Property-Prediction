#!/usr/bin/env python3
"""Strict nested affine calibration of a masked C001-style Eea parent."""
...
def nested_split(y, groups, outer_train, outer_validation, dense, sparse_parts, fingerprints, y_global, global_indices, config):
    ...
    affine = np.column_stack([inner_parent, np.ones(len(inner_parent), dtype=np.float64)])
    slope, affine_intercept = np.linalg.lstsq(affine, y[outer_train], rcond=None)[0]
    ...
    candidate = reference.clip_prediction(y[outer_train], float(slope) * outer_parent + float(affine_intercept))
    return {
        "parent": outer_parent,
        "candidate": candidate,
        "slope": float(slope),
...
def main() -> None:
    ...
    for fold in range(5):
        ...
        result = nested_split(y, groups, training, validation, dense, sparse_parts, fingerprints, y_global, indices, config)
        ...
        calibration_rows.append({"fold": fold, "slope": result["slope"], "affine_intercept": result["affine_intercept"], "inner_parent_r2": result["inner_blend_r2"], "blend_name": result["blend_name"]})
    ...
    panels["exact_archive_duplicate_group"] = {"rows": int(np.sum(duplicate)), "delta_r2": panel_delta(y, baseline, candidate, duplicate), "status": "evaluable" if int(np.sum(duplicate)) >= 5 else "inapplicable_zero_support"}
    singleton = ~duplicate
    panels["sparse_singleton_support"] = {"rows": int(np.sum(singleton)), "delta_r2": panel_delta(y, baseline, candidate, singleton), "status": "evaluable" if panel_delta(y, baseline, candidate, singleton) is not None else "support_incomplete"}
    ...
    source_hashes = {"script": sha256_file(Path(__file__).resolve()), "reference_module": sha256_file(root / "tools" / "initial_reference_pipeline.py"), "metric_plumbing": sha256_file(root / "tools" / "round2_eea_cross_target_oof_residual_stack.py")}
    audit = {
        "schema_version": "ppp.round2.eea-affine-calibration-run.v1",
        "experiment_id": run_dir.name,
        "created_at": datetime.now().astimezone().isoformat(),
        "parent": "R2-C039-MASKED-EEA-PARENT-GENERATED-IN-RUN",
        "official_inputs": inputs,
        "target": TARGET,
        "model": "nested masked C001-style blend followed by OLS affine calibration",
        "baseline_r2_nested_parent": baseline_r2,
        "candidate_r2_affine": candidate_r2,
        "delta_r2": delta_r2,
    ...
    pd.DataFrame({"canonical_no_stereo_group": groups, "fold": main_folds, "scaffold": scaffolds, "nearest_tanimoto": nearest, "y": y, "nested_parent": baseline, "affine_candidate": candidate}).to_csv(run_dir / "oof.csv", index=False)
    pd.DataFrame(fold_rows).to_csv(run_dir / "folds.csv", index=False)
    write_json(run_dir / "metrics.json", audit)
    write_json(run_dir / "config.json", {"schema_version": "ppp.round2.eea-affine-calibration.v1", "seed": 2026, "outer_folds": 5, "inner_folds": 4, "affine_fit": "OLS on inner OOF parent", "official_inputs": inputs, "source_hashes": source_hashes})
    ...
    manifest.extend([f"{source_hashes['script']}  SOURCE tools/round2_eea_affine_calibration.py", f"{source_hashes['reference_module']}  SOURCE tools/initial_reference_pipeline.py", f"{source_hashes['metric_plumbing']}  SOURCE tools/round2_eea_cross_target_oof_residual_stack.py"])
