#!/usr/bin/env python3
"""C227: deterministic compound audit v15 with C226 Nc priority."""
...
import round2_c225_clean_component_compound_audit_v14 as c225


SCHEMA = "ppp.round2.c227.clean-component-compound-audit.v15"
C226_ID = "R2-C226-20260805-0607-nc-c180-transfer-guard-v1"
CHANGED_FACTOR = (
    "Insert C226 as first Nc priority under the normal component gate, while preserving C225's guarded "
    "C224/C222/C220/C216/C218 priorities and all existing selected components."
)
...
def guarded_metric_passes(metrics: dict[str, Any], target: str) -> tuple[bool, str]:
    return c225.guarded_metric_passes(metrics, target)


def rewrite_manifest(run_dir: Path, source_hashes: dict[str, str]) -> None:
    helper = c225.c223.c221.c219.c217.base.c200
    manifest = [
...
def post_patch(run_dir: Path) -> None:
    ...
    metrics["component_priority"] = c225.c223.c221.c219.c217.base.COMPONENT_PRIORITY
    metrics["selection_rule"] = (
        "first completed clean-passing target component in frozen priority order; C226 first for Nc under "
        "the normal component gate, then C224/C222/C218 and prior fallbacks under their existing guards; "
        "no local_eval/public feedback; no same-OOF max search"
    )
    source_hashes = dict(metrics.get("source_hashes", {}))
    source_hashes["base_c213_runner"] = helper.sha256_file(Path(c225.c223.c221.c219.c217.base.__file__).resolve())
    source_hashes["c217_wrapper_runner"] = helper.sha256_file(Path(c225.c223.c221.c219.c217.__file__).resolve())
    source_hashes["c219_wrapper_runner"] = helper.sha256_file(Path(c225.c223.c221.c219.__file__).resolve())
    source_hashes["c221_wrapper_runner"] = helper.sha256_file(Path(c225.c223.c221.__file__).resolve())
    source_hashes["c223_wrapper_runner"] = helper.sha256_file(Path(c225.c223.__file__).resolve())
    source_hashes["c225_wrapper_runner"] = helper.sha256_file(Path(c225.__file__).resolve())
    ...
    write_json(
        run_dir / "config.json",
        {
            ...
            "component_priority": c225.c223.c221.c219.c217.base.COMPONENT_PRIORITY,
...
def main() -> None:
    ...
    base = c225.c223.c221.c219.c217.base
