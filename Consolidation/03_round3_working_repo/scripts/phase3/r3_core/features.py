...
# ---------------------------------------------------------------------------
# Polar moieties + charge separation + 3D conformer blocks
# (ask sec 1.1 polar group density; EDA: 3D-conformer polarizability,
#  Gasteiger sigma/pi charge separation)
# ---------------------------------------------------------------------------
...
_POLAR_SMARTS = {
    ...
    "carbonyl_alpha": "[CX3](=[OX1])[#6]",
    "sulfonyl": "[#16](=[OX1])",
}


def polar_moieties_block(mols: list) -> np.ndarray:
    """Exact counts (log1p) of polar functional moieties -- C-F, carbonyls,
    esters, ethers, sulfones, amides, siloxane motifs, ... (ask sec 1.1)."""
    patterns = {}
    for name, smarts in _POLAR_SMARTS.items():
        try:
            ...
            patterns[name] = None
    out = np.zeros((len(mols), len(patterns)), dtype=np.float32)
    for i, mol in enumerate(mols):
        for j, (name, pat) in enumerate(patterns.items()):
            if pat is None:
                continue
            try:
                out[i, j] = math.log1p(len(mol.GetSubstructMatches(pat)))
            except Exception:
                pass
    return out


def gasteiger_separation_block(mols: list) -> np.ndarray:
    """Gasteiger sigma/pi charge-separation sums + dipole-orientation proxies
    (ask EDA: charge separation for electronic property prediction)."""
    rows = []
    for mol in mols:
        try:
            ...
            for a in mol.GetAtoms():
                ...
                if hyb in (Chem.HybridizationType.SP, Chem.HybridizationType.SP2):
                    sp2 += abs(v)
                else:
                    sp3 += abs(v)
                if a.GetAtomicNum() == 6:
            ...
            for b in mol.GetBonds():
                bond_pol += abs(qarr[b.GetBeginAtomIdx()] * qarr[b.GetEndAtomIdx()])
            rows.append([
                float(qarr.sum()), float(qarr.max()), float(qarr.min()),
                float(np.abs(qarr).sum()), float(sp2), float(sp3),
                sp2 / max(sp3, 1e-9), float(hetero), float(carbon),
                hetero / max(carbon, 1e-9), float(bond_pol),
            ])
        except Exception:
            rows.append([0.0] * 11)
...
def conformer3d_block(smiles_list: list, n_confs: int = 1) -> np.ndarray:
    """3D-conformer descriptors (ETKDG + UFF; ask EDA: polarizability-relevant).
    Returns AUTOCORR3D/RDF/MORSE/WHIM/GETAWAY columns; failed embeds -> zeros.
    Slow (10-30 min for 12k rows) -- used by LONG experiments only.
    """
    from rdkit.Chem import AllChem

    mols = parse_mols(smiles_list)
    n3d = 20 + 20 + 20 + 12 + 12
    out = np.zeros((len(mols), n3d), dtype=np.float32)
    for i, mol in enumerate(mols):
        try:
            mol = Chem.AddHs(mol)
            ...
            ok = AllChem.EmbedMolecule(mol, params) == 0
            if ok:
                AllChem.UFFOptimizeMolecule(mol, maxIters=500)
                col = 0
                blocks3d = [
                    rdMolDescriptors.CalcAUTOCORR3D(mol, 0, 20),
                    rdMolDescriptors.CalcRDF(mol, 0, 20),
                    rdMolDescriptors.CalcMORSE(mol, 0, 20),
                    rdMolDescriptors.CalcWHIM(mol),
                    rdMolDescriptors.CalcGETAWAY(mol),
                ]
                for vals in blocks3d:
                    for v in vals:
                        if col >= n3d:
                            break
                        try:
