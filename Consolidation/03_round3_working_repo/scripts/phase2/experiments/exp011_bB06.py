#!/usr/bin/env python3
"""R3-BB06 [bB06] - Phase B: Selection repair on the clean arm bank. Experiment 11/150. Real grouped-CV pipeline; reads ONLY official Dataset/ inputs."""
...
def run_experiment(output_dir: Path, smoke: bool = False, data_dir: Path | None = None) -> dict[str, object]:
    ...
    # char-resid: base + char n-gram block
    def feature_fn(texts):
        texts = list(texts)
        base = f.full_feature_stack(texts, use_svd=True, svd_dim=64)
        char = f.char_tfidf(texts, ngram_range=(2, 6), max_features=2500)
